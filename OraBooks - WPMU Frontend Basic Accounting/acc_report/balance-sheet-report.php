...
<div class="container mx-auto px-4 py-6 text-gray-800">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100 no-print">
        <form id="balance-sheet-search-form" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div class="md:col-span-2">
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Start
                    Date</label>
                <input type="date" id="balance_start_date" value="<?php echo date('Y-m-01'); ?>"
            ...
            <div class="md:col-span-2">
                ...
                <input type="date" id="balance_end_date" value="<?php echo date('Y-m-d'); ?>"
                    class="w-full rounded-lg border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2.5 px-3 border bg-gray-50">
            </div>
            <div>
                <button type="submit"
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 no-print">
        <div class="relative w-full md:w-1/3">
            ...
            <input type="search" id="balanceTableSearch"
    ...
    <!-- Period Display -->
    <div id="balance-report-header" class="mb-4 text-center hidden">
        <h2 class="text-xl font-bold text-gray-800">Balance Sheet</h2>
        <p class="text-sm text-gray-600 italic mt-1" id="balance-period-text"></p>
    </div>

    <!-- Table -->
    <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="balance-sheet-table">
                ...
                <tbody id="balance-report-table-body" class="bg-white divide-y divide-gray-200">
                    <tr>
                        <td colspan="3" class="px-6 py-10 text-center text-gray-500">Click Generate to load balance data
                        </td>
                    </tr>
                </tbody>
            </table>
...
<script>
    jQuery(document).ready(function ($) {
        ...
        $('#balanceTableSearch').on('keyup', function () {
            const value = $(this).val().toLowerCase();
            $('#balance-report-table-body tr').filter(function () {
                $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
            });
        });

        $('#balance-sheet-search-form').on('submit', function (e) {
            e.preventDefault();

            const start = $('#balance_start_date').val();
            const end = $('#balance_end_date').val();

            const ajaxData = {
                action: 'search_balance_sheet_report',
            ...
            $.ajax({
                ...
                data: ajaxData,
                beforeSend: function () {
                    $('#balance-report-table-body').html('<tr><td colspan="3" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl text-indigo-500"></i><div class="mt-2 text-sm font-medium">Measuring financial health...</div></td></tr>');
                },
                success: function (res) {
                    $('#balance-report-table-body').html(res);
                    $('#balance-period-text').text('For the period of ( Transaction date ): ' + start + ' to ' + end);
                    $('#balance-report-header').removeClass('hidden');
                },
                error: function (xhr, status, error) {
                    $('#balance-report-table-body').html('<tr><td colspan="3" class="px-6 py-10 text-center text-red-500">Failed to load report: ' + error + '</td></tr>');
                }
            });
        });
        ...
        function getTableData() {
            ...
            $('#balance-sheet-table thead tr th').each(function () { if ($(this).is(':visible')) headers.push($(this).text().trim()); });
            data.push(headers);
            $('#balance-report-table-body tr').each(function () {
                const row = [];
                $(this).find('td').each(function () { if ($(this).is(':visible')) row.push($(this).text().trim()); });
                if (row.length > 0 && !row[0].includes('Click Generate') && !row[0].includes('No assets')) data.push(row);
            });
            return data;
        }

        $('#balancePrintBtn').on('click', function () {
            const printWindow = window.open('', '', 'height=600,width=900');
            const start = $('#balance_start_date').val();
            const end = $('#balance_end_date').val();
            printWindow.document.write('<html><head><title>Balance Sheet</title>');
            printWindow.document.write('<style>body{font-family:sans-serif;font-size:12px;padding:20px;}table{width:100%;border-collapse:collapse;margin-top:20px;}th,td{border:1px solid #ddd;padding:8px;text-align:left;}th{background-color:#f4f4f4;} .text-right{text-align:right;} .bg-gray-50{background:#f9fafb;} .font-bold{font-weight:bold;} .font-extrabold{font-weight:800;} .pl-10{padding-left:40px;} .uppercase{text-transform:uppercase;}</style></head><body>');
            printWindow.document.write('<h1 style="margin-bottom:5px;">Balance Sheet</h1>');
        ...
        $('#balancePdfBtn').on('click', function () {
        ...
        function exportPDF() {
            ...
            const start = $('#balance_start_date').val();
            const end = $('#balance_end_date').val();
            doc.setFontSize(18); doc.text('Balance Sheet', 14, 15);
        ...
        $('#balanceExcelBtn').on('click', function () {
