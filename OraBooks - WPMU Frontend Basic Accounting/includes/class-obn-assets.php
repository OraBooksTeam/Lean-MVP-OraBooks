...
class OBN_Assets {
    ...
    public function run_depreciation() {
        ...
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            OBN_Fiscal_Period_Posting_Guard::assert_can_post_or_fail($period_date);
        }
    ...
    public function dispose_asset() {
        ...
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            OBN_Fiscal_Period_Posting_Guard::assert_can_post_or_fail($disposal_date);
        }
    ...
    public function ajax_sync_asset_journal() {
        ...
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            global $wpdb;
            $asset = $wpdb->get_row($wpdb->prepare("SELECT purchase_date FROM {$wpdb->prefix}orabooks_ac_assets WHERE id = %d", $asset_id));
            if (!$asset) {
    ...
    public function update_asset() {
        ...
        $purchase_date = sanitize_text_field($_POST['purchase_date']);
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            $existing_asset = $wpdb->get_row($wpdb->prepare("SELECT purchase_date FROM $table WHERE id = %d", $id));
            if (!$existing_asset) {
                wp_send_json_error('Asset not found.');
            }
    ...
    public function delete_asset() {
        ...
        global $wpdb;
        $id = intval($_POST['id']);
        $asset = $wpdb->get_row($wpdb->prepare("SELECT purchase_date FROM {$wpdb->prefix}orabooks_ac_assets WHERE id = %d", $id));
        if (!$asset) {
            wp_send_json_error('Asset not found.');
        }
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
