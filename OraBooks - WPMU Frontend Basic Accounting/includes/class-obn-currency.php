...
class OBN_Currency {
    ...
    public static function insert_currency() {
        ...
        $code = strtoupper( sanitize_text_field( $_POST['currency_code'] ?? '' ) );
        ...
        if ( $exists > 0 ) {
            wp_send_json_error('Currency already exists.');
        }
        ...
        if ( $inserted ) {
            wp_send_json_success( array( 'message' => 'Currency added successfully.' ) );
        } else {
