...
class Frontend_Accounting_Sales
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_nopriv_search_sales_items', [$this, 'handle_search_sales_items']);
        add_action('wp_ajax_generate_sales_code', [$this, 'handle_generate_sales_code']);
        add_action('wp_ajax_insert_acc_sale', [$this, 'handle_insert_acc_sale']);
        ...
        add_action('wp_ajax_as_search_customers', [$this, 'handle_as_search_customers']);
        add_action('wp_ajax_nopriv_as_search_customers', [$this, 'handle_as_search_customers']);
    ...
    public function handle_as_search_customers()
    {
        ...
        $rows = $wpdb->get_results($wpdb->prepare(
            "SELECT id, customer_name FROM {$wpdb->prefix}orabooks_db_customers WHERE customer_name LIKE %s AND status=1 ORDER BY customer_name ASC LIMIT 20",
            $like
        ));
    ...
    public function handle_search_sales_items()
    {
        ...
        $sql = "SELECT id, item_name, item_code, sku, sales_price, purchase_price, tax_id, hsn, category_id, brand_id, stock, item_image 
                FROM $table 
    ...
    public function handle_insert_acc_sale()
    {
        check_ajax_referer('frontend_ajax_nonce', 'security');
        ...
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            OBN_Fiscal_Period_Posting_Guard::assert_can_post_or_fail($sales_date);
        }
        // $sale_code = $wpdb->get_var($wpdb->prepare("SELECT sales_code FROM $table WHERE id = %d", $sales_id));

        // Define variables needed for customer handling
        $created_by = get_current_user_id();
        $system_ip = sanitize_text_field($_SERVER['REMOTE_ADDR']);
        $table_cust = $wpdb->prefix . 'orabooks_db_customers';

        $warehouse_id = intval($_POST['warehouse_id'] ?? 0);
        $customer_id = intval($_POST['customer_id'] ?? 0);
        $customer_name = sanitize_text_field($_POST['customer_name_input'] ?? '');

        // If customer name is blank, use or create a default "Walking Customer"
        if (empty($customer_name) && empty($customer_id)) {
            // Try to find existing walking customer (case-insensitive)
            $walk_id = $wpdb->get_var($wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}orabooks_db_customers WHERE LOWER(customer_name) = %s LIMIT 1",
                'walking customer'
            ));
            if ($walk_id) {
                $customer_id = intval($walk_id);
            } else {
                // Insert a new Walking Customer record
                $store_id = intval($_POST['store_id'] ?? 1);
                $count_id = $wpdb->get_var($wpdb->prepare(
                    "SELECT MAX(count_id) FROM {$wpdb->prefix}orabooks_db_customers WHERE store_id = %d",
                    $store_id
                ));
                $count_id = ($count_id) ? intval($count_id) + 1 : 1;
                $prefix = $wpdb->get_var($wpdb->prepare(
                    "SELECT customer_init FROM {$wpdb->prefix}orabooks_db_store WHERE id = %d",
                    $store_id
                ));
                if (!$prefix)
                    $prefix = 'CUS-';
                ...
                $wpdb->insert($table_cust, $cust_data);
                $customer_id = $wpdb->insert_id;
            }
        }

        // If a new customer name is provided (not walking)
        // if (!empty($customer_name) && strtolower($customer_name) !== 'walking customer') {
        if (!empty($customer_name) && empty($customer_id)) {
            // Check if customer already exists with this name (case-insensitive)
            $existing_cust_id = $wpdb->get_var($wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}orabooks_db_customers WHERE LOWER(customer_name) = LOWER(%s) LIMIT 1",
                $customer_name
            ));
            if ($existing_cust_id) {
                $customer_id = intval($existing_cust_id);
            } else {
                // Insert new customer
                $store_id = intval($_POST['store_id'] ?? 1);
                $table_cust = $wpdb->prefix . 'orabooks_db_customers';
                $count_id = $wpdb->get_var($wpdb->prepare("SELECT MAX(count_id) FROM $table_cust WHERE store_id = %d", $store_id));
                $count_id = ($count_id) ? intval($count_id) + 1 : 1;
        ...
        if (is_array($items) && count($items) > 0) {
            foreach ($items as $item) {
                // Auto-create item if no item_id (manually typed item)
                if (empty($item['item_id'])) {
                    ...
                    if (!empty($item_name)) {
                        ...
                        if ($existing_item_id) {
                            $item['item_id'] = intval($existing_item_id);
                        } else {
                            // Generate new item code
                            $count_id = $wpdb->get_var($wpdb->prepare("SELECT MAX(count_id) FROM {$wpdb->prefix}orabooks_db_items WHERE store_id = %d", $data['store_id']));
                            $count_id = ($count_id) ? intval($count_id) + 1 : 1;
                            $prefix = $wpdb->get_var($wpdb->prepare("SELECT item_init FROM {$wpdb->prefix}orabooks_db_store WHERE id = %d", $data['store_id']));
                            if (!$prefix)
                                $prefix = 'ITM-';
                            ...
                            $new_item_data = [
                                ...
                                'item_code' => $item_code,
                                'item_name' => $item_name,
                                'sales_price' => floatval($item['unit_price'] ?? 0),
                                'stock' => 0,
                                'status' => 1,
                                'created_date' => current_time('Y-m-d'),
                                'created_time' => current_time('H:i:s'),
                            ...
                            $wpdb->insert($wpdb->prefix . 'orabooks_db_items', $new_item_data);
                            $item['item_id'] = $wpdb->insert_id;
                        }
                    } else {
                        continue;
                    }
        ...
        wp_send_json_success(['message' => $msg, 'sale_id' => $last_id, 'customer_id' => $customer_id]);
    }
    ...
    private function create_sales_journal_entry($sales_id)
    {
        ...
        if ($paid_amount > 0) {
            ...
            $raw_acc_id = $payment ? intval($payment->account_id) : 0;
            $cash_account_id = $accounts['cash_bank'];
            if ($raw_acc_id > 0) {
                // If it is from orabooks_ac_accounts, convert to coa_list ID via account_code
                $acc_code = $wpdb->get_var($wpdb->prepare("SELECT account_code FROM {$wpdb->prefix}orabooks_ac_accounts WHERE id = %d", $raw_acc_id));
                if ($acc_code) {
                    $real_coa_id = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}orabooks_ac_coa_list WHERE account_code = %s AND status=1 LIMIT 1", $acc_code));
                    if ($real_coa_id) {
                        $cash_account_id = $real_coa_id;
                    }
                } else {
                    // Maybe it is already a coa_list ID
                    $cash_account_id = $raw_acc_id;
                }
            }
    ...
    public function handle_update_sale()
    {
        ...
        $sales_date = sanitize_text_field($_POST['sales_date'] ?? current_time('Y-m-d'));
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            $existing_sale = $wpdb->get_row($wpdb->prepare("SELECT sales_date FROM {$wpdb->prefix}orabooks_db_sales WHERE id = %d", $sales_id));
            if (!$existing_sale) {
                ...
                if (empty($item['item_id'])) {
                    ...
                    if (!empty($item_name)) {
                        ...
                        if ($existing_item_id) {
                            ...
                            // Generate new item code
                            $count_id = $wpdb->get_var($wpdb->prepare("SELECT MAX(count_id) FROM {$wpdb->prefix}orabooks_db_items WHERE store_id = %d", 1));
                            $count_id = ($count_id) ? intval($count_id) + 1 : 1;
                            $prefix = $wpdb->get_var($wpdb->prepare("SELECT item_init FROM {$wpdb->prefix}orabooks_db_store WHERE id = %d", 1));
                            if (!$prefix)
                                $prefix = 'ITM-';
                            $item_code = $prefix . str_pad($count_id, 6, '0', STR_PAD_LEFT);
    ...
    public function handle_search_sales()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $r) {
                ...
                echo '<tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                        <div class="flex justify-center items-center space-x-2">
                            ';
                if (Frontend_Accounting_Permissions::has_view_permission('sales-order-list') && $r->sales_status !== 'Delivered') {
                ...
                if (Frontend_Accounting_Permissions::has_view_permission('sales-pending-delivery') && $r->sales_status !== 'Delivered') {
    ...
    {
        $dompdf_path = Frontend_Accounting_PATH . 'lib/dompdf/';
        ...
        include FRONTEND_ACCOUNTING_TEMPLATE_PATH . 'sales/sales-invoice.php';
        $html = ob_get_clean();
