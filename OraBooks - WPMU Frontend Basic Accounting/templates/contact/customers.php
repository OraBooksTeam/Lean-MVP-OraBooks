    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            ...
            <div class="flex gap-2">
                ...
                <button id="customer-toggle-form-btn" class="px-6 py-2.5 bg-gray-900 hover:bg-black text-white font-medium rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center gap-2">
        ...
        <div id="customer-form-section" class="hidden mb-8">
            <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                    <h2 class="text-lg font-bold text-gray-800" id="customer-form-title">Add New Customer</h2>
                    <button id="customer-cancel-btn" class="text-gray-400 hover:text-gray-600 transition-colors">
                ...
                <div class="p-6">
                    <form id="customer-form" class="space-y-6" enctype="multipart/form-data">
                        ...
                        <div class="flex justify-end pt-6 border-t border-gray-100">
                            <button type="submit" id="customer-save-btn" class="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transform transition hover:-translate-y-0.5">
                                Save Customer
                            </button>
                        </div>
                    </form>
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            <div class="p-4 border-b border-gray-100 bg-gray-50 flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div class="relative w-full md:w-1/3">
                    ...
                    <input type="search" id="customer-searchInput" class="block w-full pl-10 pr-3 py-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-white focus:ring-blue-500 focus:border-blue-500" placeholder="Search customers...">
                </div>

                <!-- Export & Column Buttons -->
                <div class="flex gap-2 flex-wrap justify-end w-full md:w-auto">
                    <button id="customer-printBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none" title="Print">
                        <i class="fa-solid fa-print mr-1"></i> <span class="hidden sm:inline">Print</span>
                    </button>
                    <button id="customer-pdfBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none" title="Export to PDF">
                        <i class="fa-solid fa-file-pdf mr-1 text-red-600"></i> <span class="hidden sm:inline">PDF</span>
                    </button>
                    <button id="customer-excelBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none" title="Export to Excel">
                        <i class="fa-solid fa-file-excel mr-1 text-green-600"></i> <span class="hidden sm:inline">Excel</span>
                    </button>
                    <button id="customer-csvBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none" title="Export to CSV">
                        <i class="fa-solid fa-file-csv mr-1 text-blue-600"></i> <span class="hidden sm:inline">CSV</span>
                    </button>
                    
                    <!-- Column Visibility Dropdown -->
                    <div class="relative flex-1 sm:flex-none">
                        <button id="customer-columnToggleBtn" class="w-full sm:w-auto text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors" title="Toggle Columns">
                            <i class="fa-solid fa-columns mr-1"></i> Columns
                        </button>
                        <div id="customer-columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                            <div class="p-3 space-y-2">
                                <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                                    <input type="checkbox" class="customer-column-toggle mr-2" data-column="0" checked> Customer Name
                                </label>
                                <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                                    <input type="checkbox" class="customer-column-toggle mr-2" data-column="1" checked> Contact
                                </label>
                                <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                                    <input type="checkbox" class="customer-column-toggle mr-2" data-column="2" checked> Location
                                </label>
                                <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                                    <input type="checkbox" class="customer-column-toggle mr-2" data-column="3" checked> Balance
                                </label>
                                <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                                    <input type="checkbox" class="customer-column-toggle mr-2" data-column="4" checked> Status
                                </label>
                            </div>
                        </div>
                    </div>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-center">
                                <button class="customer-toggle-status relative inline-flex items-center cursor-pointer" data-id="<?php echo $cust->id; ?>" data-status="<?php echo $cust->status; ?>">
...
jQuery(document).ready(function($) {
    ...
    fetch('<?php echo OBN_ACCOUNTING_PLUGIN_URL; ?>assets/countries.json')
        ...
        .catch(() => {
            $('#country_id, #ship_country_id').html('<option value="">Select Country</option>');
        });
    ...
    // Form Toggle
    $(document).on('click', '#customer-toggle-form-btn', function() {
        resetForm();
        $('#customer-form-section').removeClass('hidden');
        $('#customer-form-title').text('Add New Customer');
        $('html, body').animate({ scrollTop: 0 }, 'slow');
        $('#customer-save-btn').text('Save Customer');
    });

    $(document).on('click', '#customer-cancel-btn', function() {
        $('#customer-form-section').addClass('hidden');
        resetForm();
    });
    ...
    $('#customer-form').on('submit', function(e) {
        ...
        $('#customer-save-btn').prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Saving...');
        ...
        $.ajax({
            ...
            success: function(res) {
                if(res.success) {
                    ...
                    $('#customer-save-btn').prop('disabled', false).text('Save Customer');
                }
            },
            error: function() {
                alert('Server error.');
    ...
    // Toggle Status
    $('.customer-toggle-status').on('click', function() {
        let btn = $(this);
        let id = btn.data('id');
    ...
    window.editCustomer = function(cust) {
        ...
        $('#customer-save-btn').text('Update Customer');
        $('html, body').animate({ scrollTop: 0 }, 'slow');
    ...
    // Client-side Search
    $('#customer-searchInput').on('keyup', function() {
    ...
    $('#customer-columnToggleBtn').on('click', function(e) {
        e.stopPropagation();
        $('#customer-columnDropdown').toggleClass('hidden');
    });
    
    $(document).on('click', function(e) {
        if (!$(e.target).closest('#customer-columnToggleBtn, #customer-columnDropdown').length) {
            $('#customer-columnDropdown').addClass('hidden');
        }
    });
    
    $('.customer-column-toggle').on('change', function() {
        const column = $(this).data('column');
        const isChecked = $(this).is(':checked');
        
        $('#customersTable thead tr th').eq(column).toggle(isChecked);
    ...
    function getCustomersTableData() {
        const data = [];
        const headers = [];
        
        $('#customersTable thead tr th').each(function(index) {
    ...
    // Print functionality
    $('#customer-printBtn').on('click', function() {
        ...
        const tableData = getCustomersTableData();
    ...
    // PDF Export
    $('#customer-pdfBtn').on('click', function() {
        exportPDF();
    });
    ...
    // Excel Export
    $('#customer-excelBtn').on('click', function() {
    ...
    // CSV Export
    $('#customer-csvBtn').on('click', function() {
        const tableData = getCustomersTableData();
        let csv = '';
