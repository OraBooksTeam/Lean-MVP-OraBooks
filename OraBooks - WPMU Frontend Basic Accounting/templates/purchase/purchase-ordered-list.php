...
// Logic - Only Ordered purchases
$where = "WHERE p.status = 1 AND p.purchase_status = 'Ordered'";
...
$total_qty = $wpdb->get_var("SELECT SUM(purchase_qty) FROM {$wpdb->prefix}orabooks_db_purchaseitems pi LEFT JOIN {$wpdb->prefix}orabooks_db_purchase p ON pi.purchase_id = p.id WHERE p.purchase_status = 'Ordered'");
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Manage and approve purchase orders</p>
            </div>
        </div>
        <div class="flex gap-2">
            ...
            <a href="<?php echo esc_url(add_query_arg('view', 'purchase-pending-list')); ?>" class="inline-flex items-center px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-all font-medium shadow-md hover:shadow-lg active:scale-95">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="rounded-xl shadow-sm p-5 text-white" style="background: linear-gradient(135deg, #1a83db, #1569B3);">
            <h3 class="text-xs font-semibold uppercase tracking-wider opacity-80 mb-1">Total Orders</h3>
        ...
        <div class="rounded-xl shadow-sm p-5 text-white" style="background: linear-gradient(135deg, #1a83db, #1569B3);">
             <h3 class="text-xs font-semibold uppercase tracking-wider opacity-80 mb-1">Order Value</h3>
    ...
    <div class="bg-gray-50 rounded-xl border border-gray-200 p-5 mb-8">
        <form method="get" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <input type="hidden" name="view" value="purchase-ordered-list">
            <div>
    ...
    <div class="search-filter-bar mb-4 flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        ...
        <div class="export-toolbar flex gap-2 flex-wrap">
            <button id="purchaseorderedlistPrintBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Print">
                <i class="fa-solid fa-print mr-1"></i> <span class="hidden sm:inline">Print</span>
            </button>
            <button id="purchaseorderedlistPdfBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Export to PDF">
                <i class="fa-solid fa-file-pdf mr-1 text-red-600"></i> <span class="hidden sm:inline">PDF</span>
            </button>
            <button id="purchaseorderedlistExcelBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Export to Excel">
                <i class="fa-solid fa-file-excel mr-1 text-green-600"></i> <span class="hidden sm:inline">Excel</span>
            </button>
            <button id="purchaseorderedlistCsvBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Export to CSV">
            ...
            <table id="purchasesTable" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Order Code</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    <?php if (count($purchases) > 0): ?>
                        <?php foreach ($purchases as $p): ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600"><?php echo esc_html($p->purchase_code); ?></td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <span class="px-2.5 py-1 inline-flex text-xs leading-5 font-bold rounded-full bg-blue-100 text-blue-800">
                                        <i class="fa-solid fa-paper-plane mr-1"></i> Ordered
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <button type="button" class="text-green-600 hover:text-green-900 bg-green-50 p-2 rounded-lg transition-colors approve-order" data-id="<?php echo $p->id; ?>" title="Approve Order">
                                        <i class="fa-solid fa-check"></i>
                                    </button>
                                    <a href="<?php echo esc_url(add_query_arg(['view' => 'purchase-invoice', 'id' => $p->id])); ?>" class="text-blue-600 hover:text-blue-900 bg-blue-50 p-2 rounded-lg transition-colors" title="View Order">
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                    <!-- <a href="<?php //echo esc_url(add_query_arg(['view' => 'edit-purchase', 'id' => $p->id])); ?>" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-2 rounded-lg transition-colors" title="Edit Order">
                    ...
                    <?php else: ?>
                        <tr><td colspan="6" class="px-6 py-12 text-center text-gray-400">
                            <i class="fa-solid fa-clipboard-list text-5xl mb-3 block opacity-20"></i>
                            No purchase orders found with current filters.
                        </td></tr>
                    <?php endif; ?>
                </tbody>
...
jQuery(document).ready(function($) {
    ...
    $('.approve-order').on('click', function() {
        if (!confirm('Are you sure you want to approve this order? It will be moved to pending status.')) return;
        
        const id = $(this).data('id');
        const btn = $(this);
        
        $.post(Frontend_Accounting_ajax.ajax_url, {
            ...
            if (res.success) {
                ...
                alert(res.data || 'Failed to approve order');
            }
        });
    });
    ...
    // Export Functions
    function getPurchaseOrderedTableData() {
        const data = [];
        const headers = [];
    ...
    // Print
    $('#purchaseorderedlistPrintBtn').on('click', function() {
        ...
        const tableData = getPurchaseOrderedTableData();
    ...
    // PDF
    $('#purchaseorderedlistPdfBtn').on('click', function() {
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF();
        
        doc.setFontSize(18);
        doc.text('Purchase Orders', 14, 22);
    ...
    // Excel
    $('#purchaseorderedlistExcelBtn').on('click', function() {
        const tableData = getPurchaseOrderedTableData();
    ...
    // CSV
    $('#purchaseorderedlistCsvBtn').on('click', function() {
        const tableData = getPurchaseOrderedTableData();
