...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 md:mb-6 gap-3 md:gap-4">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Manage sales orders awaiting processing</p>
            </div>
        </div>
    ...
    <div class="search-filter-bar mb-4 flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        ...
        <div class="export-toolbar flex gap-2 flex-wrap">
            <button id="salesorderlistPrintBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Print">
                <i class="fa-solid fa-print mr-1"></i> <span class="hidden sm:inline">Print</span>
            </button>
            <button id="salesorderlistPdfBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Export to PDF">
                <i class="fa-solid fa-file-pdf mr-1 text-red-600"></i> <span class="hidden sm:inline">PDF</span>
            </button>
            <button id="salesorderlistExcelBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Export to Excel">
                <i class="fa-solid fa-file-excel mr-1 text-green-600"></i> <span class="hidden sm:inline">Excel</span>
            </button>
            <button id="salesorderlistCsvBtn" class="text-gray-700 bg-white border border-gray-300 hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 font-medium rounded-lg text-sm px-3 py-2 transition-colors flex-1 sm:flex-none h-[42px]" title="Export to CSV">
                    ...
                    <?php if (count($sales) > 0): ?>
                        <?php foreach ($sales as $s): ?>
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-blue-600"><?php echo esc_html($s->sales_code); ?></td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <button type="button" class="text-green-600 hover:text-green-900 bg-green-50 p-2 rounded-lg transition-colors approve-order" data-id="<?php echo $s->id; ?>" title="Approve Order">
                                        <i class="fa-solid fa-truck"></i>
                                    </button>
                                    <a href="<?php echo esc_url(add_query_arg(['view' => 'sales-invoice', 'sales_id' => $s->id])); ?>" class="text-blue-600 hover:text-blue-900 bg-blue-50 p-2 rounded-lg transition-colors" title="View Order">
                                        <i class="fa-solid fa-eye"></i>
                                    </a>
                                    <a href="<?php echo esc_url(add_query_arg(['view' => 'edit-sales', 'sale_id' => $s->id])); ?>" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 p-2 rounded-lg transition-colors" title="Edit Order">
...
jQuery(document).ready(function($) {
    ...
    $('.approve-order').on('click', function() {
        ...
        if (confirm('Are you sure you want to approve this order? This will move it to pending delivery status.')) {
            const data = {
            ...
            console.log('Approving order with data:', data);
    ...
    function getSalesOrderTableData() {
        const data = [];
        
        // Headers
        data.push(['Date', 'Order Code', 'Customer', 'Status', 'Total']);
    ...
    // Print
    $('#salesorderlistPrintBtn').on('click', function() {
        const printWindow = window.open('', '', 'height=600,width=800');
        printWindow.document.write('<html><head><title>Sales Orders</title>');
        ...
        printWindow.document.write('<h1>Sales Orders</h1>');
        
        const tableData = getSalesOrderTableData();
    ...
    // PDF Export
    $('#salesorderlistPdfBtn').on('click', function() {
    ...
    // Excel Export
    $('#salesorderlistExcelBtn').on('click', function() {
        const tableData = getSalesOrderTableData();
    ...
    // CSV Export
    $('#salesorderlistCsvBtn').on('click', function() {
        const tableData = getSalesOrderTableData();
