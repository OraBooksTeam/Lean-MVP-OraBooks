...
<div class="lg:h-[calc(100vh-100px)] h-auto flex flex-col lg:flex-row gap-4">
    ...
    <div
        class="w-full lg:w-1/2 flex flex-col bg-white rounded-xl shadow-lg border border-gray-200 overflow-visible min-h-[600px] lg:min-h-0">

        <!-- Header: Warehouse & Customer -->
        <div class="p-4 bg-gray-50 border-b border-gray-200 space-y-3">
            <div class="flex justify-between items-center">
                <h2 class="font-bold text-gray-700 flex items-center mb-1 md:mb-0"><i
                        class="fa-solid fa-cash-register mr-2 text-blue-600"></i> <span class="hidden sm:inline">POS
                        Terminal</span><span class="sm:hidden">POS</span></h2>
                <div class="flex items-center gap-2">
                    <span class="text-[16px] text-gray-400 font-medium">CODE:</span>
                    <input type="text" id="pos_sales_code" readonly
                        class="text-[14px] bg-transparent border-none text-left text-gray-500 p-0 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 px-2 py-2"
            ...
            <div class="grid grid-cols-2 gap-2">
                ...
                <div class="flex">
                    <input type="text" id="pos_customer_name"
                        class="w-full text-sm rounded-l border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Customer Name (type to search)...">
                    <input type="hidden" id="pos_customer_id" value="">
                    <button type="button" onclick="openCustomerModal()" id="pos-add-customer-btn"
                        ...
                        title="Add New Customer"><i class="fa-solid fa-plus text-lg"></i></button>
                </div>
            </div>
            ...
            <div class="flex gap-2">
                <div class="relative flex-1">
                    ...
                    <input type="search" id="cart-item-search"
                        class="w-full pl-10 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm py-2"
                        placeholder="Search by name, code, SKU or barcode...">
                </div>
                <button type="button" id="pos-add-item-btn"
                    class="bg-blue-100 text-blue-600 px-3 rounded-lg border border-gray-300 hover:bg-blue-200 transition-colors flex items-center justify-center flex-shrink-0"
                    title="Add New Item">
                ...
                <div class="flex justify-between text-gray-600 items-center">
                    ...
                    <div class="flex w-24">
                        <input type="number" id="pos_discount_input"
                            class="w-full h-6 text-right text-xs border-gray-300 rounded focus:ring-blue-500" value=""
                            placeholder="0.00">
                    </div>
                </div>
            ...
            <div class="space-y-2 mb-3">
                <select id="pos_payment_type" class="w-full text-xs rounded border-gray-300 py-2">
                    ...
                    <?php foreach ($payment_types as $pt):
                        echo "<option value='{$pt->id}'>{$pt->payment_type}</option>";
                    endforeach; ?>
                </select>
                ...
                <div id="bkash_section" class="hidden">
                    <input type="tel" id="pos_bkash_number" class="w-full text-xs rounded border-gray-300 py-2"
                        placeholder="Enter bKash number (01xxxxxxxxx)" maxlength="11"
                        oninput="this.value = this.value.replace(/[^0-9]/g, '')">
                </div>
                <div id="nagad_section" class="hidden">
                    <input type="tel" id="pos_nagad_number" class="w-full text-xs rounded border-gray-300 py-2"
                        placeholder="Enter Nagad number (01xxxxxxxxx)" maxlength="11"
                        oninput="this.value = this.value.replace(/[^0-9]/g, '')">
                </div>
                <div id="check_section" class="hidden">
                    <input type="text" id="pos_bank_name" class="w-full text-xs rounded border-gray-300 py-2 mb-2"
                        placeholder="Enter bank name">
                    <input type="text" id="pos_check_number" class="w-full text-xs rounded border-gray-300 py-2"
                        placeholder="Enter check number">
                </div>
            </div>

            <div class="flex gap-2">
                <button type="button" id="btn-reset-pos"
                    class="px-4 py-3 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 font-medium">
                    <i class="fa-solid fa-trash"></i>
                </button>
                <button type="button" id="btn-pay-now"
                    class="flex-1 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-bold shadow-md flex justify-center items-center">
                    <span>PAY NOW</span>
    ...
    <div
        class="w-full lg:w-1/2 flex flex-col bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden min-h-[600px] lg:min-h-0">
        <!-- Filters -->
        <div class="p-4 border-b border-gray-200 bg-gray-50">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="relative">
                    ...
                    <input type="search" id="grid-search"
                        class="w-full pl-10 rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm py-2"
                        placeholder="Search by name, code, SKU or barcode...">
                </div>
                <select id="grid-category"
                    class="rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm py-2">
                    <option value="">All Categories</option>
                    <?php foreach ($categories as $c):
                        echo "<option value='{$c->id}'>{$c->category_name}</option>";
                    endforeach; ?>
                </select>
                <select id="grid-brand"
                    class="rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 text-sm py-2">
...
<template id="pos-row-tpl">
    <tr class="bg-white border-b hover:bg-gray-50 group">
        <td class="px-3 py-2 align-middle">
            <div class="font-medium text-gray-800 text-xs md:text-sm truncate max-w-[80px] md:max-w-[120px] item-name">
            </div>
            <div class="text-[9px] md:text-[10px] text-gray-500 item-code"></div>
        </td>
        <td class="px-2 py-2 text-center hidden md:table-cell">
            <span
                class="inline-block px-2 py-1 bg-gray-100 text-gray-600 rounded text-[10px] font-medium item-stock"></span>
        </td>
        <td class="px-2 py-2 align-middle">
            <div class="flex items-center justify-center bg-gray-100 rounded border border-gray-200">
                <button
                    class="w-4 h-4 md:w-6 md:h-6 flex items-center justify-center text-gray-500 hover:text-red-500 hover:bg-gray-200 rounded-l transition-colors btn-dec text-[10px] md:text-sm">-</button>
                <input type="text"
                    class="w-5 md:w-8 h-4 md:h-6 text-center text-[9px] md:text-xs bg-transparent border-0 p-0 focus:ring-0 item-qty"
                    value="1" readonly>
                <button
                    class="w-4 h-4 md:w-6 md:h-6 flex items-center justify-center text-gray-500 hover:text-green-500 hover:bg-gray-200 rounded-r transition-colors btn-inc text-[10px] md:text-sm">+</button>
            </div>
        </td>
        <td class="px-2 py-2 text-right text-gray-600 text-[9px] md:text-xs item-price">0.00</td>
        <td class="px-2 py-2 text-right">
            <input type="number" step="0.01"
                class="w-10 md:w-16 h-5 md:h-6 text-right text-[9px] md:text-xs border-gray-300 rounded focus:ring-blue-500 item-discount"
                value="0">
        </td>
...
<script>
    jQuery(document).ready(function ($) {
        ...
        $('#pos_customer_name').autocomplete({
            source: function (request, response) {
                $.post(ajaxurl, { action: 'as_search_customers', term: request.term, security: nonce }, function (res) {
                    if (res.success) {
                        response(res.data.map(function (c) {
                            return { label: c.customer_name, value: c.customer_name, data: c };
                        }));
                    } else response([]);
                }, 'json');
            },
            ...
            select: function (event, ui) {
                $('#pos_customer_id').val(ui.item.data.id);
                $('#pos_customer_name').val(ui.item.label);
                return false;
            }
        });
        // Clear hidden customer_id when manual name entry changes (to allow new customer creation)
        $('#pos_customer_name').on('input', function () { $('#pos_customer_id').val(''); });
        ...
        function generateSalesCode() {
            $.post(ajaxurl, { action: 'generate_sales_code', security: nonce }, function (res) {
                if (res.success) $('#pos_sales_code').val(res.data.code);
            }, 'json');
        }
        ...
        function renderGrid(products) {
            ...
            products.forEach(p => {
                const img = p.item_image ? p.item_image : 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22150%22 height=%22150%22%3E%3Crect width=%22150%22 height=%22150%22 fill=%22%23f0f0f0%22/%3E%3Ctext x=%2250%%22 y=%2250%%22 dominant-baseline=%22middle%22 text-anchor=%22middle%22 fill=%22%23999%22 font-size=%2214%22%3ENo%20Image%3C/text%3E%3C/svg%3E';
                const card = `
        ...
        // Open item modal for POS
        $('#pos-add-item-btn').on('click', function () { openPosSaleItemModal(); });

        // Callback for New Item Modal
        window.openPosSaleItemModal = function () {
            const modal = $('#item-modal-pos');
        ...
        $('#btn-pay-now').on('click', function () {
            ...
            // First, generate a new sales code
            $.post(ajaxurl, { action: 'generate_sales_code', security: nonce }, function (res) {
                if (!res.success) {
                    alert('Failed to generate sales code');
                    return;
                }
                var sales_code = res.data.code; // get generated code
                $('#sales_code').val(sales_code); // update input field
                ...
                if (!$('#check_section').hasClass('hidden')) {
                    if (!$('#pos_bank_name').val()) { alert('Enter bank name'); return; }
                    if (!$('#pos_check_number').val()) { alert('Enter check number'); return; }
                }
                ...
                const payload = {
                    ...
                    customer_id: $('#pos_customer_id').val() || null,
                    customer_name_input: $('#pos_customer_name').val(),
                    ...
                    items_json: JSON.stringify(cart.map(i => {
                        ...
                        return { item_id: i.id, name: i.name, account_id: i.account_id, qty: i.qty, unit_price: i.price, discount: i.discount || 0, tax_amt: taxAmount, total: lineTotal };
                    }))
                };
                ...
                $.post(ajaxurl, payload, function (res) {
                    if (res.success) {
                        ...
                        let errMsg = typeof res.data === 'string' ? res.data : (res.data.message || 'Unknown error');
                        alert('Error: ' + errMsg);
...
<?php include FRONTEND_ACCOUNTING_TEMPLATE_PATH . 'modals/modal_item_pos.php'; ?>