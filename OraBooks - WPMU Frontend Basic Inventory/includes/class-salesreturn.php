...
class Frontend_Inventory_SalesReturn {
    ...
    public function handle_update_salesreturn() {
        ...
        $existing_return = $wpdb->get_row($wpdb->prepare("SELECT return_status, sales_id FROM {$wpdb->prefix}orabooks_db_salesreturn WHERE id=%d", $id));
        ...
        $data = [
            'warehouse_id'      => $warehouse_id,
            'reference_no'      => sanitize_text_field($_POST['reference_no'] ?? ''),
            'return_date'       => sanitize_text_field($_POST['sales_date'] ?? ''),
            'customer_id'       => $customer_id,
            'other_charges_input'   => $other_charges_input,
            'other_charges_tax_id'  => $other_charges_tax_id,
            'other_charges_amt'     => $other_charges_amt,
