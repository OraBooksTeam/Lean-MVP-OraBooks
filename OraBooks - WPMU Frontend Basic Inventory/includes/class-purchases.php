...
class Frontend_Inventory_Purchases
{
    ...
    public function handle_update_purchase()
    {
        ...
        if (is_array($items)) {
            ...
            foreach ($items as $item) {
                ...
                $wpdb->insert($wpdb->prefix . 'orabooks_db_purchaseitems', $item_data);
    ...
    public function handle_delete_purchase()
    {
        ...
        $purchase = $wpdb->get_row($wpdb->prepare("SELECT warehouse_id, purchase_status FROM {$wpdb->prefix}orabooks_db_purchase WHERE id=%d", $id));
    ...
    public function handle_insert_purchase()
    {
        ...
        $items_json = wp_kses_post($_POST['items_json'] ?? '[]');
    ...
    private function create_purchase_journal_entry($purchase_id)
    {
        ...
        // If the payment account_id is from orabooks_ac_accounts, we might need a mapping or just fallback to coa_list
