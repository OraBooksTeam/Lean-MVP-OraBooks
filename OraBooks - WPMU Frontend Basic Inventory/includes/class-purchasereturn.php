...
class Frontend_Inventory_PurchaseReturn
{
    ...
    public function handle_update_purchasereturn()
    {
        ...
        $existing_return = $wpdb->get_row($wpdb->prepare("SELECT return_status, purchase_id FROM {$wpdb->prefix}orabooks_db_purchasereturn WHERE id=%d", $id));
        if (!$existing_return)
        ...
        $data = [
            'warehouse_id' => $warehouse_id,
            'reference_no' => sanitize_text_field($_POST['reference_no'] ?? ''),
            'return_date' => sanitize_text_field($_POST['purchase_date'] ?? ''),
            'supplier_id' => $supplier_id,
            'other_charges_input' => $other_charges_input,
            'other_charges_tax_id' => $other_charges_tax_id,
            'other_charges_amt' => $other_charges_amt,
