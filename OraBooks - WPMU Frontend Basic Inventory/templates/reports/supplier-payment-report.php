...
$payment_types = $wpdb->get_results("SELECT payment_type FROM {$wpdb->prefix}orabooks_db_paymenttypes WHERE status = 1 ORDER BY payment_type ASC");
...
<div class="container mx-auto px-4 py-6 text-gray-800">
    <div class="flex justify-between items-center mb-6 no-print">
        <h1 class="text-2xl font-bold text-gray-800">Supplier Payment Report</h1>
    </div>

    <!-- Filters -->
    <div class="bg-white rounded-lg shadow-sm p-5 mb-6 border border-gray-200 no-print">
        <form id="sp-search-form" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
            ...
            <div>
                ...
                <select id="payment_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2 px-3 border bg-gray-50">
                    <option value="">All Types</option>
                    <?php foreach ($payment_types as $pt): ?>
                        <option value="<?php echo esc_attr($pt->payment_type); ?>"><?php echo esc_html($pt->payment_type); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
            ...
            <div class="relative">
                ...
                <div id="columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    <div class="p-3 space-y-2 text-sm">
                        ...
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="1" checked> Purchase Invoice No.
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="2" checked> Payment Date
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="3" checked> Supplier ID
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="4" checked> Supplier Name
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="5" checked> Payment Type
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="6" checked> Payment Note
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="7" checked> Paid Payment
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Purchase Invoice No.</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Payment Date</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Supplier ID</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Supplier Name</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Payment Type</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Payment Note</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Paid Payment(<?php echo $currency; ?>)</th>
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">User</th>
                    </tr>
                </thead>
                <tbody id="sp-table-body" class="bg-white divide-y divide-gray-200">
                     <tr>
                ...
                <tfoot class="bg-gray-100 font-bold text-gray-700">
                    <tr>
                        <td colspan="7" class="px-6 py-3 text-right">Summary Totals:</td>
                        <td id="foot-paid" class="px-6 py-3 text-right">0.00</td>
                        <td></td>
                    </tr>
                </tfoot>
            </table>
...
jQuery(document).ready(function($) {
    ...
    function calculateFooter() {
        let paid = 0;
        $('#sp-table-body tr:visible').each(function() {
            const p = parseFloat($(this).find('.payment-amt').text().replace(/,/g, '')) || 0;
            paid += p;
        });
    ...
    $('#sp-search-form').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'search_supplier_payment_report',
            ...
            beforeSend: function() {
                $('#sp-table-body').html('<tr><td colspan="9" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl"></i><div class="mt-2">Loading...</div></td></tr>');
            },
            success: function(res) {
                $('#sp-table-body').html(res);
                calculateFooter();
            }
        });
    });
    ...
    $('#printBtn').on('click', function() {
        ...
        printWindow.document.write('<html><head><title>Supplier Payment Report</title>');
        ...
        printWindow.document.write('<h1>Supplier Payment Report</h1>');
    ...
    function exportPDF() {
        ...
        doc.text('Supplier Payment Report', 14, 15);
        const tableData = getTableData();
    ...
    function exportExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'SupplierPayments');
        XLSX.writeFile(wb, 'supplier-payment-report.xlsx');
    }
