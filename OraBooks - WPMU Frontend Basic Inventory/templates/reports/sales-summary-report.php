...
<div class="container mx-auto px-4 py-6 text-gray-800">
    ...
    <div class="bg-white rounded-lg shadow-sm p-5 mb-6 border border-gray-200 no-print">
        <form id="ss-search-form" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
            ...
            <div class="relative">
                ...
                <div id="columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    <div class="p-3 space-y-2 max-h-60 overflow-y-auto text-sm">
                        ...
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="1" checked> Item Name
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="2" checked> Category
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="3" checked> Total QTY
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="4" checked> Total Amount
                        </label>
                    </div>
                </div>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider">Total QTY</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Total Amount</th>
                    </tr>
                </thead>
                <tbody id="ss-table-body" class="bg-white divide-y divide-gray-200">
                     <tr>
                        <td colspan="5" class="px-6 py-10 text-center text-gray-500">Click Search to load data</td>
                    </tr>
                </tbody>
                <tfoot class="bg-gray-100 font-bold text-gray-700">
                    <tr>
                        <td colspan="3" class="px-6 py-3 text-right">Grand Totals:</td>
...
jQuery(document).ready(function($) {
    ...
    function calculateFooter() {
        ...
        $('#ss-table-body tr:visible').each(function() {
            const q = parseFloat($(this).find('.qty').text().replace(/,/g, '')) || 0;
            const tt = parseFloat($(this).find('.total-amt').text().replace(/,/g, '')) || 0;
            qty += q; total += tt;
        });
    ...
    $('#tableSearch').on('keyup', function() {
        ...
        $('#ss-table-body tr').filter(function() {
            $(this).toggle($(this).text().toLowerCase().indexOf(value) > -1)
        });
        calculateFooter();
    });

    $('#ss-search-form').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'search_sales_summary_report',
            ...
            beforeSend: function() {
                $('#ss-table-body').html('<tr><td colspan="5" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl"></i><div class="mt-2">Loading...</div></td></tr>');
            },
            success: function(res) {
                $('#ss-table-body').html(res);
                calculateFooter();
            }
        });
    });
    ...
    $('#printBtn').on('click', function() {
        ...
        printWindow.document.write('<html><head><title>Sales Summary Report</title>');
        ...
        printWindow.document.write('<h1>Sales Summary Report</h1>');
    ...
    function exportExcel() {
        ...
        XLSX.writeFile(wb, 'sales-summary-report.xlsx');
    }
