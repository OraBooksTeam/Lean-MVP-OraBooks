...
<div class="container mx-auto px-4 py-6 text-gray-800">
    <div class="flex justify-between items-center mb-6 no-print">
        ...
        <div class="flex gap-2">
            <button onclick="window.print()" class="bg-gray-800 hover:bg-gray-900 text-white font-medium py-2 px-4 rounded transition-colors duration-200">
    ...
    <div class="bg-white rounded-lg shadow-sm p-5 mb-6 border border-gray-200 no-print">
        <form id="stock-search-form" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            <div>
            ...
            <div>
                ...
                <select id="category_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2 px-3 border bg-gray-50">
                    ...
                    <?php foreach ($categories as $c): ?>
                        <option value="<?php echo $c->id; ?>"><?php echo esc_html($c->category_name); ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
            ...
            <div class="relative">
                ...
                <div id="columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    <div class="p-3 space-y-2">
                        ...
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="1" checked> Item Code
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="2" checked> Item Name
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                        ...
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="5" checked> Unit Price
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="6" checked> Tax
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="7" checked> Purchase Cost
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="8" checked> Sales Price
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="9" checked> Current Stock
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="10" checked> Stock Value (By Sale Price)
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded text-sm">
                            <input type="checkbox" class="column-toggle mr-2" data-column="11" checked> Stock Value (By Purchase Price)
                        </label>
                    </div>
                </div>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Item Code</th>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">Tax</th>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider">Current Stock</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Stock Value<br>(By Sale Price)</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Stock Value<br>(By Purchase Price)</th>
                    </tr>
                </thead>
                ...
                <tfoot class="bg-gray-100 font-bold text-gray-700">
                    <tr>
                        <td colspan="9" class="px-6 py-3 text-right">Total Summary:</td>
                        <td id="foot-stock" class="px-6 py-3 text-center">0.00</td>
                        <td id="foot-value-sales" class="px-6 py-3 text-right">0.00</td>
                        <td id="foot-value-purchase" class="px-6 py-3 text-right">0.00</td>
                    </tr>
                </tfoot>
            </table>
        </div>
...
jQuery(document).ready(function($) {
    ...
    function calculateFooter() {
        let totalStock = 0, totalValSales = 0, totalValPur = 0;
        
        $('#stock-table-body tr:visible').each(function() {
            const s = parseFloat($(this).find('.current-stock').text().replace(/,/g, '')) || 0;
            const vs = parseFloat($(this).find('.stock-value-sales').text().replace(/,/g, '')) || 0;
            const vp = parseFloat($(this).find('.stock-value-purchase').text().replace(/,/g, '')) || 0;
            totalStock += s; 
            totalValSales += vs; 
            totalValPur += vp;
        });

        $('#foot-stock').text(totalStock.toLocaleString('en-US', {minimumFractionDigits: 2}));
        $('#foot-value-sales').text(totalValSales.toLocaleString('en-US', {minimumFractionDigits: 2}));
        $('#foot-value-purchase').text(totalValPur.toLocaleString('en-US', {minimumFractionDigits: 2}));
    }
    ...
    $('#stock-search-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            beforeSend: function() {
                $('#stock-table-body').html('<tr><td colspan="12" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl"></i><div class="mt-2">Loading...</div></td></tr>');
            },
            success: function(res) {
                $('#stock-table-body').html(res);
                calculateFooter();
            }
        });
    });
    ...
    $('.column-toggle').on('change', function() {
        ...
        // Footer handling
        if(column == 9) $('#foot-stock').toggle(isChecked);
        if(column == 10) $('#foot-value-sales').toggle(isChecked);
        if(column == 11) $('#foot-value-purchase').toggle(isChecked);
    });
    ...
    function getTableData() {
        ...
        // Adjust footer to match visible columns if needed, but usually we just want the totals.
        // For simplicity, let's just add the footer row as is if it has content.
        if (footer.length > 0) {
            // data.push(footer); // Optional: append footer to export
        }
