...
<div class="container mx-auto px-4 py-6 text-gray-800">
    <div class="flex justify-between items-center mb-6 no-print">
        <h1 class="text-2xl font-bold text-gray-800">Customer Due Report</h1>
    </div>

    <!-- Filters -->
    <div class="bg-white rounded-lg shadow-sm p-5 mb-6 border border-gray-200 no-print">
        <form id="cd-search-form" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div>
            ...
            <div class="relative">
                ...
                <div id="columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-50">
                    <div class="p-3 space-y-2 text-sm">
                        ...
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="1" checked> Customer
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                        ...
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="4" checked> Grand Total
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="5" checked> Paid Amount
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Grand Total</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Paid Amount</th>
                        <th class="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider">Due</th>
                    </tr>
                </thead>
                <tbody id="cd-table-body" class="bg-white divide-y divide-gray-200">
                     <tr>
...
jQuery(document).ready(function($) {
    ...
    function calculateFooter() {
        ...
        $('#cd-table-body tr:visible').each(function() {
            const t = parseFloat($(this).find('.sales-amount').text().replace(/,/g, '')) || 0;
    ...
    $('#tableSearch').on('keyup', function() {
        ...
        $('#cd-table-body tr').filter(function() {
    ...
    $('#cd-search-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'search_customer_due_report',
            ...
            beforeSend: function() {
                $('#cd-table-body').html('<tr><td colspan="7" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl"></i><div class="mt-2">Loading...</div></td></tr>');
            },
            success: function(res) {
                $('#cd-table-body').html(res);
                calculateFooter();
            }
        });
    });
    ...
    $('#printBtn').on('click', function() {
        ...
        printWindow.document.write('<html><head><title>Customer Due Report</title>');
        ...
        printWindow.document.write('<h1>Customer Due Report</h1>');
    ...
    function exportExcel() {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'CustomerDue');
        XLSX.writeFile(wb, 'customer-due-report.xlsx');
    }
