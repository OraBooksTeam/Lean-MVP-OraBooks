...
<div class="container mx-auto px-4 py-6 text-gray-800">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100 no-print">
        <form id="search-form" class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
            <div class="md:col-span-2">
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Select Account</label>
                <select id="account_id" class="w-full rounded-lg border-gray-200 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2.5 px-3 border bg-gray-50 transition-all duration-200 outline-none">
                    <option value="">-- Choose Account --</option>
                    <?php if ($accounts) : foreach ($accounts as $acc) : ?>
                        <option value="<?php echo $acc->id; ?>"><?php echo esc_html($acc->account_code . ' - ' . $acc->account_name); ?></option>
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 no-print">
        <div class="relative w-full md:w-1/3">
            ...
            <input type="search" id="tableSearch" class="block w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-xl leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 sm:text-sm shadow-sm" placeholder="Search transactions...">
        </div>
    ...
    <div id="report-header" class="mb-4 text-center hidden">
        <h2 class="text-xl font-bold text-gray-800" id="account-title">Ledger Report</h2>
            ...
            <table class="min-w-full divide-y divide-gray-200" id="ledger-table">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider w-32">Debit (<?php echo $currency; ?>)</th>
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider w-32">Credit (<?php echo $currency; ?>)</th>
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider w-32">Balance (<?php echo $currency; ?>)</th>
                    </tr>
                </thead>
                <tbody id="report-table-body" class="bg-white divide-y divide-gray-200">
                     <tr>
                        <td colspan="6" class="px-6 py-10 text-center text-gray-500">Select an account and click Generate</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
jQuery(document).ready(function($) {
    ...
    $('#search-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            data: ajaxData,
            beforeSend: function() {
                $('#report-table-body').html('<tr><td colspan="6" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl text-indigo-500"></i><div class="mt-2 text-sm font-medium">Fetching details...</div></td></tr>');
            },
            success: function(res) {
                $('#report-table-body').html(res);
                $('#account-title').text('Ledger: ' + $('#account_id option:selected').text());
                $('#period-text').text('For the period of ( Transaction date ): ' + start + ' to ' + end);
                $('#report-header').removeClass('hidden');
            },
            error: function(xhr, status, error) {
                $('#report-table-body').html('<tr><td colspan="6" class="px-6 py-10 text-center text-red-500">Failed to load: ' + error + '</td></tr>');
            }
        });
    });
    ...
    $('#printBtn').on('click', function() {
        ...
        const start = $('#start_date').val();
        const end = $('#end_date').val();
        const account = $('#account_id option:selected').text();
    ...
    function exportPDF() {
        ...
        doc.autoTable({ 
            ...
            didParseCell: function(data) {
                if (data.row.index >= 0) {
                    // If row length was shorter than header, it might need alignment adjustment
                    // But AutoTable handles colSpan objects well.
                }
            }
        });
