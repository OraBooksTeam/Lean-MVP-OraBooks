            ...
            <table class="min-w-full divide-y divide-gray-200" id="balance-sheet-table">
                <thead class="bg-indigo-500 text-white">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider">Account Total (<?php echo $currency; ?>)</th>
                        <th class="px-6 py-4 text-right text-xs font-bold uppercase tracking-wider">Total (<?php echo $currency; ?>)</th>
                    </tr>
                </thead>
                <tbody id="report-table-body" class="bg-white divide-y divide-gray-200">
                     <tr>
                        <td colspan="3" class="px-6 py-10 text-center text-gray-500">Click Generate to load balance data</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
jQuery(document).ready(function($) {
    ...
    $('#search-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            data: ajaxData,
            beforeSend: function() {
                $('#report-table-body').html('<tr><td colspan="3" class="px-6 py-10 text-center text-gray-500"><i class="fa-solid fa-spinner fa-spin text-2xl text-indigo-500"></i><div class="mt-2 text-sm font-medium">Measuring financial health...</div></td></tr>');
            },
            ...
            error: function(xhr, status, error) {
                $('#report-table-body').html('<tr><td colspan="3" class="px-6 py-10 text-center text-red-500">Failed to load report: ' + error + '</td></tr>');
            }
        });
    });
