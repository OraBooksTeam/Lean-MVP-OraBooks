...
<div class="container mx-auto px-4 py-6">
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="<?php echo esc_url(add_query_arg('view', 'transfer-list')); ?>"
        ...
        <div class="p-6">
            <form id="transfer-form" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        <label for="warehouse_from" class="block text-sm font-medium text-gray-700 mb-1">From Warehouse
                            *</label>
                        <select name="warehouse_from" id="warehouse_from" required
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2 px-3 border bg-gray-50">
                    ...
                    <div>
                        <label for="warehouse_to" class="block text-sm font-medium text-gray-700 mb-1">To Warehouse
                            *</label>
                ...
                <!-- Search -->
                <div>
                    <label for="item-autocomplete" class="block text-sm font-medium text-gray-700 mb-1">Search Item
                        (from Source Warehouse)</label>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <i class="fa-solid fa-magnifying-glass text-gray-400"></i>
                        </div>
                        <!-- <input type="search" id="item-autocomplete" class="w-full pl-10 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm py-2 px-3 border bg-gray-50 disabled:bg-gray-100 disabled:cursor-not-allowed" placeholder="Search item by name / code..." disabled> -->
                        <input type="search" id="item-autocomplete"
                            ...
                            placeholder="Search item by name / code..." disabled>
                    </div>
                    <p class="mt-1 text-xs text-red-500 hidden" id="warehouse-error">Please select 'From Warehouse'
                        first.</p>
                </div>
                        ...
                        <tbody id="items-tbody" class="bg-white divide-y divide-gray-200">
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-gray-500">
                                    <span class="block text-sm">No items. Select source warehouse and search.</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                    <a href="<?php echo esc_url(add_query_arg('view', 'transfer-list')); ?>"
                        class="px-4 py-2 bg-white text-gray-700 font-medium rounded-md border border-gray-300 hover:bg-gray-50">Cancel</a>
                    <button type="submit" id="submit-btn"
                        class="px-6 py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 items-center flex">
                        <i class="fa-solid fa-paper-plane mr-2"></i> Save Transfer
                    </button>
                </div>
            </form>
        </div>
    ...
    jQuery(document).ready(function ($) {
        ...
        $('#item-autocomplete').autocomplete({
            source: function (request, response) {
                ...
                $.ajax({
                    ...
                    data: { action: 'search_stock_items', security: '<?php echo wp_create_nonce('frontend_ajax_nonce'); ?>', search: request.term, warehouse_id: wh },
        ...
        $('#transfer-form').on('submit', function (e) {
            ...
            $.ajax({
                ...
                success: function (r) {
                    if (r.success) { alert('Saved!'); window.location.href = '<?php echo esc_url(add_query_arg('view', 'transfer-list')); ?>'; }
