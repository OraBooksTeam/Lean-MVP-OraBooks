...
<div class="p-6">
    ...
    <div id="form-section" class="hidden bg-white rounded-lg shadow-md p-6 mb-6 border border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2 mb-4" id="form-title">Add New Brand</h3>
        
        <form id="brand-form" class="space-y-4">
            <input type="hidden" name="action" value="frontend_save_brand">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- Brand Code -->
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">Brand Code</label>
                    <input type="text" name="brand_code" id="brand_code" class="w-full bg-gray-100 text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none cursor-not-allowed" readonly>
                </div>

                <!-- Brand Name -->
                <div>
                    <label class="block text-gray-700 text-sm font-bold mb-2">Brand Name <span class="text-red-500">*</span></label>
                    <input type="text" name="brand_name" id="brand_name" required class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500 placeholder-gray-400">
                </div>
            </div>
    ...
    <div class="search-filter-bar mb-4 flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        <div class="relative flex-1 w-full">
            ...
            <input type="search" id="searchInput" class="block w-full pl-10 pr-3 py-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search brands...">
        </div>
                ...
                <div id="columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <div class="p-3 space-y-2">
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="0" checked> Brand Code
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="1" checked> Brand Name
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-x-auto border border-gray-200">
        <table id="brandsTable" class="min-w-full divide-y divide-gray-200">
            <thead class="bg-indigo-600 text-white">
                <tr>
                    <th scope="col" class="px-6 py-3">Brand Code</th>
                ...
                <?php if (!empty($brands)) : ?>
                    <?php foreach ($brands as $brand) : ?>
                        <tr class="bg-white border-b hover:bg-gray-50 text-gray-700">
                            <td class="px-6 py-4 font-medium"><?php echo esc_html($brand->brand_code); ?></td>
                            <td class="px-6 py-4 font-bold text-gray-900"><?php echo esc_html($brand->brand_name); ?></td>
                            <td class="px-6 py-4"><?php echo esc_html($brand->description); ?></td>
                            <td class="px-6 py-4">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer toggle-status" data-id="<?php echo $brand->id; ?>" <?php checked($brand->status, 1); ?>>
                                    <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                                </label>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <button class="text-blue-600 hover:text-blue-900 mr-3 edit-brand" 
                                    data-id="<?php echo $brand->id; ?>"
                                    data-code="<?php echo esc_attr($brand->brand_code); ?>"
                                    data-name="<?php echo esc_attr($brand->brand_name); ?>"
                                    data-desc="<?php echo esc_attr($brand->description); ?>">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <button class="text-red-600 hover:text-red-900 delete-brand" data-id="<?php echo $brand->id; ?>">
...
jQuery(document).ready(function($) {
    ...
    function resetForm() {
        ...
        $('#brand_id').val('');
        $('#brand_code').val(''); 
        $('#form-title').text('Add New Brand');
        $('#save-btn').html('<i class="fa fa-save mr-2"></i> Save Brand');
    }
    ...
    $('#toggle-form-btn').on('click', function() {
        ...
        generateCode(); // Generate code for new entry
    ...
    $(document).on('click', '.edit-brand', function() {
        ...
        $('#save-btn').html('<i class="fa fa-save mr-2"></i> Update Brand');
        $('#form-section').removeClass('hidden');
        $('html, body').animate({ scrollTop: 0 }, 'slow');
    });
    ...
    $('.column-toggle').on('change', function() {
        ...
        // Toggle header
        $('#brandsTable thead tr th').eq(column).toggle(isChecked);
        // Toggle cells
    ...
    function getTableData() {
        ...
        $('#brandsTable thead tr th').each(function(index) {
    ...
    $('#printBtn').on('click', function() {
        ...
        printWindow.document.write('<html><head><title>Brands List</title>');
        ...
        printWindow.document.write('<h1>Brands List</h1>');
    ...
    $('#excelBtn').on('click', function() {
        ...
        XLSX.writeFile(wb, 'brands-list.xlsx');
    });
    ...
    $('#csvBtn').on('click', function() {
        ...
        link.setAttribute('download', 'brands-list.csv');
