...
<div class="p-6">
    ...
    <div id="form-section" class="hidden bg-white rounded-lg shadow-md p-6 mb-6 border border-gray-200">
        <h3 class="text-lg font-semibold text-gray-800 border-b border-gray-200 pb-2 mb-4" id="form-title">Add New Unit</h3>
        ...
        <form id="unit-form" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="unit_name" id="unit_name" required class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500 placeholder-gray-400">
            </div>
    ...
    <div class="search-filter-bar mb-4 flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        <div class="relative flex-1 w-full">
            ...
            <input type="search" id="searchInput" class="block w-full pl-10 pr-3 py-2 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Search units...">
        </div>
                ...
                <div id="columnDropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
                    <div class="p-3 space-y-2">
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="0" checked> Unit Name
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="1" checked> Description
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="2" checked> Status
                        </label>
                        <label class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded">
                            <input type="checkbox" class="column-toggle mr-2" data-column="3" checked> Actions
                        </label>
                    </div>
                </div>
            </div>
            ...
            <tbody id="units-table-body">
                <?php if (!empty($units)) : ?>
                    <?php foreach ($units as $unit) : ?>
                        <tr class="bg-white border-b hover:bg-gray-50 text-gray-700">
                            <td class="px-6 py-4 font-bold text-gray-900"><?php echo esc_html($unit->unit_name); ?></td>
                            <td class="px-6 py-4"><?php echo esc_html($unit->description); ?></td>
                            <td class="px-6 py-4">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer toggle-status" data-id="<?php echo $unit->id; ?>" <?php checked($unit->status, 1); ?>>
                                    <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                                </label>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <button class="text-blue-600 hover:text-blue-900 mr-3 edit-unit" 
                                    data-id="<?php echo $unit->id; ?>"
                                    data-name="<?php echo esc_attr($unit->unit_name); ?>"
                                    data-desc="<?php echo esc_attr($unit->description); ?>">
                                    <i class="fa-solid fa-pen-to-square"></i>
                                </button>
                                <button class="text-red-600 hover:text-red-900 delete-unit" data-id="<?php echo $unit->id; ?>">
                ...
                <?php else : ?>
                    <tr class="bg-white border-b">
                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">No units found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
jQuery(document).ready(function($) {
    ...
    function resetForm() {
        $('#unit-form')[0].reset();
        $('#unit_id').val('');
        $('#form-title').text('Add New Unit');
        $('#save-btn').html('<i class="fa fa-save mr-2"></i> Save Unit');
    }
    ...
    $(document).on('click', '.edit-unit', function() {
        ...
        $('#form-title').text('Edit Unit');
        $('#save-btn').html('<i class="fa fa-save mr-2"></i> Update Unit');
        $('#form-section').removeClass('hidden');
        $('html, body').animate({ scrollTop: 0 }, 'slow');
    });
    ...
    $('.column-toggle').on('change', function() {
        ...
        // Toggle header
        $('#unitsTable thead tr th').eq(column).toggle(isChecked);
        // Toggle cells
    ...
    function getTableData() {
        ...
        $('#unitsTable thead tr th').each(function(index) {
            if($(this).is(':visible') && index < 3) { // Exclude Actions (index 3)
    ...
    $('#printBtn').on('click', function() {
        ...
        printWindow.document.write('<html><head><title>Units List</title>');
        ...
        printWindow.document.write('<h1>Units List</h1>');
    ...
    $('#csvBtn').on('click', function() {
        ...
        link.setAttribute('download', 'units-list.csv');
