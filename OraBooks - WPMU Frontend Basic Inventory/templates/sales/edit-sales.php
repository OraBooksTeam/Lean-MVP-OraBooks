...
if (!$sales_id) {
    echo "<div class='p-4 bg-red-100 text-red-700'>Error: Sales ID is required.</div>";
    return;
}
...
if (!$sale) {
    echo "<div class='p-4 bg-red-100 text-red-700'>Error: Sale not found.</div>";
    return;
}
...
// Fetch Items from salesitems table
$items_results = $wpdb->get_results($wpdb->prepare(
    "SELECT si.*, i.item_code, i.sku, i.stock 
...
if ($items_results) {
    foreach ($items_results as $row) {
        // Calculate approximate tax percent from stored amounts
        // line_before = sales_qty * price_per_unit
        // taxable = line_before - discount_amt
        // tax_percent = (tax_amt / taxable) * 100
        $taxable = ($row->sales_qty * $row->price_per_unit) - $row->discount_amt;
        $tax_percent = 0;
        if ($taxable > 0) {
            $tax_percent = ($row->tax_amt / $taxable) * 100;
        }
...
// Fetch existing payment info for this sale
$existing_payment = $wpdb->get_row($wpdb->prepare(
    "SELECT payment_type, account_id, payment_note FROM {$wpdb->prefix}orabooks_db_salespayments WHERE sales_id = %d ORDER BY id DESC LIMIT 1",
    $sales_id
));
?>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-3 md:p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 md:mb-6 gap-3 md:gap-4">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Modify existing sales invoice (<?php echo esc_html($sale->sales_code); ?>)</p>
            </div>
        </div>
        <a href="?view=view-sales" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-medium shadow-md hover:shadow-lg">
    ...
    <form id="sales-form" class="space-y-3 md:space-y-6 relative">
        ...
        <input type="hidden" name="sales_id" value="<?php echo esc_attr($sales_id); ?>">
        <input type="hidden" name="store_id" value="<?php echo esc_attr($sale->store_id); ?>">

        <!-- Top Section: Sale Info -->
        <div class="bg-gray-50 rounded-lg p-3 md:p-5 border border-gray-200">
            <h2 class="text-lg font-semibold text-gray-800 mb-3 md:mb-4 flex items-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
                <div>
                    ...
                    <div class="flex rounded-lg shadow-sm group">
                        <span class="inline-flex items-center px-4 rounded-l-lg border border-r-0 border-gray-300 bg-gray-100 text-gray-500 transition-colors min-h-[35px] md:min-h-0">
                            <i class="fa-solid fa-hashtag"></i>
                        </span>
                        <input type="text" name="sales_code" id="sales_code" value="<?php echo esc_attr($sale->sales_code); ?>" readonly class="flex-1 block w-full rounded-none rounded-r-lg border-gray-300 bg-gray-100 text-gray-600 cursor-not-allowed focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11 px-3 text-sm min-h-[35px] md:min-h-0">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="warehouse_id" id="warehouse_id" required class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11">
                        <?php foreach ($warehouses as $w): ?>
                            <option value="<?php echo esc_attr($w->id); ?>" <?php selected($sale->warehouse_id, $w->id); ?>><?php echo esc_html($w->warehouse_name); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        <select name="customer_id" id="customer_id" required class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11">
                            <option value="">- Select Customer -</option>
                            <?php foreach ($customers as $c): ?>
                                <option value="<?php echo esc_attr($c->id); ?>" <?php selected($sale->customer_id, $c->id); ?>><?php echo esc_html($c->customer_name); ?></option>
                            <?php endforeach; ?>
                        </select>
                        <button type="button" class="w-10 md:w-12 h-10 md:h-11 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-200 transition-colors flex items-center justify-center flex-shrink-0" title="Add New Customer" onclick="openCustomerModal()">
                ...
                <div>
                    ...
                    <input type="text" name="reference_no" id="reference_no" value="<?php echo esc_attr($sale->reference_no); ?>" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11 px-3" placeholder="e.g. PO-123">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Sales Date</label>
                    <input type="date" name="sales_date" id="sales_date" value="<?php echo esc_attr($sale->sales_date); ?>" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11 px-3">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                    <input type="date" name="due_date" id="due_date" value="<?php echo esc_attr($sale->due_date); ?>" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11 px-3">
                </div>
                ...
                <div>
                    ...
                    <select name="sales_status" id="sales_status"
                        ...
                        <option value="Delivered" <?php echo ($sale->sales_status === 'Delivered') ? 'selected' : ''; ?>>Delivered</option>
                        <option value="Pending" <?php echo ($sale->sales_status === 'Pending') ? 'selected' : ''; ?>>Pending</option>
                        <option value="Ordered" <?php echo ($sale->sales_status === 'Ordered') ? 'selected' : ''; ?>>Ordered</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
            <div class="p-3 md:p-4 bg-gray-50 border-b border-gray-200 flex flex-col md:flex-row justify-between items-center gap-3 md:gap-4">
                <div class="w-full md:w-auto flex-1 max-w-2xl flex gap-2">
                    <div class="flex-1 flex rounded-lg shadow-sm group">
                        <div class="inline-flex items-center px-4 rounded-l-lg border border-r-0 border-gray-300 bg-white text-gray-400 group-focus-within:text-blue-500 group-focus-within:border-blue-500 transition-colors">
                            <i class="fa-solid fa-search"></i>
                        </div>
                        <input type="text" id="item-autocomplete" class="flex-1 block w-full rounded-none rounded-r-lg border-gray-300 leading-5 bg-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-0 sm:text-sm h-11 px-3 py-2" placeholder="Search items by name, code or SKU...">
                    </div>
                    <button type="button" class="w-12 h-11 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-200 transition-colors flex items-center justify-center flex-shrink-0" title="Add New Item" onclick="openItemModal()">
                        <i class="fa-solid fa-plus-circle text-xl"></i>
                    </button>
                </div>
                <button type="button" id="add-blank-row" class="w-full md:w-auto px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-medium shadow-sm h-11 flex items-center justify-center">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[1000px]" id="items-table">
                    ...
                    <tbody id="items-tbody" class="divide-y divide-gray-100">
                        <tr id="empty-row-msg" style="display: none;">
                        ...
                        <!-- rows appended dynamically via scripts below -->
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
            ...
            <div class="bg-gray-50 rounded-lg p-3 md:p-5 border border-gray-200 h-full">
                ...
                <div class="flex justify-between items-center p-3 bg-white rounded-lg border border-gray-200 mb-3 shadow-sm">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" step="0.01" name="other_charges_input" id="other_charges_input" value="<?php echo esc_attr($sale->other_charges_input); ?>" class="w-[60%] rounded-l-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11 px-3">
                            <select name="other_charges_tax_id" id="other_charges_tax_id" class="w-[40%] rounded-r-lg border-l-0 border-gray-300 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 text-sm h-10 md:h-11 px-2">
                                <option value="">No Tax</option>
                                <?php foreach ($taxes as $t): ?>
                                    <option value="<?php echo esc_attr($t->id); ?>" data-percent="<?php echo esc_attr($t->tax); ?>" <?php selected($sale->other_charges_tax_id, $t->id); ?>><?php echo esc_html($t->tax_name . ' (' . $t->tax . '%)'); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" step="0.01" id="discount_to_all_input" name="discount_to_all_input" value="<?php echo esc_attr($sale->discount_to_all_input); ?>" class="w-[60%] rounded-l-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11 px-3">
                            <select id="discount_to_all_type" name="discount_to_all_type" class="w-[40%] rounded-r-lg border-l-0 border-gray-300 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 text-sm h-10 md:h-11 px-2">
                                <option value="Percentage" <?php selected($sale->discount_to_all_type, 'Percentage'); ?>>Percentage %</option>
                                <option value="Fixed" <?php selected($sale->discount_to_all_type, 'Fixed'); ?>>Fixed Amount</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="sales_note" id="sales_note" rows="3" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm p-3" placeholder="Add a note..."><?php echo esc_textarea($sale->sales_note); ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-3 md:p-5 border border-gray-200 h-full">
                ...
                <div class="space-y-3 mb-6">
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Subtotal</span>
                        <span class="font-bold text-gray-800" id="subtotal_text"><?php echo number_format($sale->subtotal, 2); ?></span>
                    </div>
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Other Charges</span>
                        <span class="font-bold text-gray-800" id="other_charges_text"><?php echo number_format($sale->other_charges_amt, 2); ?></span>
                    </div>
                    <div class="flex justify-between text-sm text-red-600">
                        <span>Discount on All</span>
                        <span class="font-bold">- <span id="discount_all_text"><?php echo number_format($sale->tot_discount_to_all_amt, 2); ?></span></span>
                    </div>
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Tax Total</span>
                    ...
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Round Off</span>
                        <span class="font-bold text-gray-800" id="round_off_text"><?php echo number_format($sale->round_off, 2); ?></span>
                    </div>
                    <div class="border-t border-gray-300 pt-3 flex justify-between text-lg font-bold text-gray-900 items-end">
                        <span>Grand Total</span>
                        <span class="text-2xl text-green-600" id="grand_total_text"><?php echo number_format($sale->grand_total, 2); ?></span>
                    </div>
                </div>
                ...
                <div class="bg-white p-3 md:p-4 rounded-lg border border-blue-100 shadow-sm mb-4">
                    <h3 class="text-sm font-semibold text-blue-800 mb-3 flex items-center">
                        <i class="fa-solid fa-wallet mr-2"></i> Payment Information
                    </h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                         <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Payment Status</label>
                            <div class="p-2 md:py-2.5 bg-gray-50 rounded border text-sm font-semibold h-10 md:h-11 flex items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        <div>
                            ...
                            <select id="payment_type_id" name="payment_type_id" class="w-full rounded border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11">
                                <option value="">- Select -</option>
                                <?php foreach ($payment_types as $pt): ?>
                                    <option value="<?php echo esc_attr($pt->id); ?>" <?php selected($existing_payment ? $existing_payment->payment_type : 0, $pt->id); ?>>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="payment_amount" id="payment_amount" value="<?php echo esc_attr($sale->paid_amount); ?>" placeholder="0.00" class="w-full rounded border-2 border-blue-400 text-sm focus:ring-blue-500 focus:border-blue-500 font-semibold text-blue-700 bg-blue-50/30 h-10 md:h-11 px-3">
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        <div>
                            <label class="block text-xs font-medium text-gray-500 mb-1">Account</label>
                            <select id="account_id" name="account_id" class="w-full rounded border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11">
                                <option value="">- Select -</option>
                                <?php foreach ($coa_accounts as $ca): ?>
                                    <option value="<?php echo esc_attr($ca->id); ?>" <?php selected($existing_payment ? $existing_payment->account_id : 0, $ca->id); ?>>
                        ...
                        <div>
                            ...
                            <textarea name="payment_note" id="payment_note" rows="1" class="w-full rounded border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 min-h-[44px] leading-tight py-2 px-3"><?php echo esc_textarea($existing_payment ? $existing_payment->payment_note : ''); ?></textarea>
                        </div>
                    </div>
                </div>

                <button type="button" id="save-sale" class="w-full py-4 md:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all font-bold text-lg shadow-md flex justify-center items-center">
                    <i class="fa-solid fa-save mr-2"></i> Update Sale
                </button>
            </div>
        </div>
    </form>
...
<template id="row-template">
    <tr class="bg-white border-b hover:bg-gray-50 transition-colors" data-item-id="">
        <td class="p-3">
            ...
            <input type="text" class="w-full border-gray-200 rounded text-sm row-name focus:border-blue-400 focus:ring-blue-400 transition-all font-medium text-gray-800" placeholder="Type to search item...">
        ...
        <td class="p-3">
             <select class="row-account-id w-full rounded border-gray-300 text-xs focus:ring-blue-500 focus:border-blue-500 bg-transparent">
        ...
        <td class="p-3">
            <div class="flex items-center">
                <button type="button" class="qty-dec w-6 h-8 bg-gray-200 hover:bg-gray-300 rounded-l flex items-center justify-center text-gray-600">-</button>
                <input type="number" min="0.01" step="0.01" class="w-16 h-8 border-y border-gray-300 text-center text-sm row-qty focus:ring-0 focus:outline-none" value="1">
                <button type="button" class="qty-inc w-6 h-8 bg-gray-200 hover:bg-gray-300 rounded-r flex items-center justify-center text-gray-600">+</button>
            </div>
        </td>
        <td class="p-3">
        ...
        <td class="p-3">
            <input type="number" step="0.01" class="w-16 rounded border-gray-300 text-sm row-tax" value="0" readonly tabindex="-1">
        </td>
         <td class="p-3">
            <input type="text" class="w-20 rounded border-gray-300 bg-gray-50 text-sm row-tax-amt cursor-default" value="0.00" readonly tabindex="-1">
        </td>
        <td class="p-3 font-medium text-gray-900">
             <input type="text" class="w-24 rounded border-gray-300 bg-gray-50 text-sm font-semibold row-total cursor-default" value="0.00" readonly tabindex="-1">
        </td>
        <td class="p-3 text-center">
...
jQuery(document).ready(function($) {
    ...
    const initialItems = <?php echo json_encode($items_data); ?>;
    ...
    function addItemRow(item) {
        ...
        if (item.qty) row.find('.row-qty').val(item.qty);
        if (item.discount) row.find('.row-discount').val(item.discount);
    ...
    $('#save-sale').on('click', function(){
        ...
        $.post(ajaxurl, payload, function(res){
            ...
            btn.prop('disabled', false).html('<i class="fa-solid fa-save mr-2"></i> Update Sale');
        });
    });
