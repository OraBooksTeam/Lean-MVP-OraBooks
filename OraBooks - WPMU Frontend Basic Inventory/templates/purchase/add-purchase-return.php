    ...
    jQuery(document).ready(function ($) {
        ...
        $.get(ajaxurl, { action: 'get_purchase_details', id: original_purchase_id, security: nonce }, function (res) {
            if (res.success) {
                ...
                $('#reference_no').val('RTN-' + p.purchase_code);

                $('#other_charges_input').val(p.other_charges_input || 0);
                $('#other_charges_tax_id').val(p.other_charges_tax_id || '');
                $('#discount_to_all_input').val(p.discount_to_all_input || 0);
                $('#discount_to_all_type').val(p.discount_to_all_type || 'Percentage');
        ...
        $('#save-purchase').on('click', function () {
            ...
            if ($('#items-tbody tr').length === 0) { alert('Your return is empty. Please add items.'); return; }
