                    ...
                    <div class="w-full max-w-xl relative flex gap-2">
                        <div class="relative flex-1">
                            ...
                            <input type="text" id="item-autocomplete"
                                class="flex-1 block w-full rounded-none rounded-r-lg border-gray-300 leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 sm:text-sm h-11 px-3 py-2"
                ...
                <table class="w-full text-left border-collapse" id="items-table">
                    <thead>
                        <tr class="text-white text-sm uppercase tracking-wider" style="background-color: #1569B3;">
                            <th class="p-3 font-semibold border-b w-1/4">Item Name</th>
                            ...
                            <th class="p-3 font-semibold border-b w-24">Tax %</th>
                    ...
                    <tbody id="items-tbody" class="divide-y divide-gray-100">
                        ...
                        <tr id="empty-row-msg">
                            <td colspan="9" class="p-8 text-center text-gray-400">
                                ...
                                Search and select items to add them to the purchase list
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    jQuery(document).ready(function ($) {
        ...
        function generateCode() {
            $.post(ajaxurl, { action: 'generate_purchase_code', security: nonce }, function (res) {
                if (res.success) $('#purchase_code').val(res.data.code);
            }, 'json');
        }
        generateCode();
        ...
        function addItemRow(item) {
            ...
            row.find('.remove-row').on('click', function () {
                ...
                if ($('#items-tbody tr:not(#empty-row-msg)').length === 0) $('#empty-row-msg').show();
                recalc();
            });
        }
        ...
        $('#save-purchase').on('click', function () {
            ...
            if (!$('#supplier_id').val()) { alert('Select supplier'); return; }
            ...
            const payload = {
                ...
                // tax_total not explicitly needed by backend but logic calculates it
            ...
            $.post(ajaxurl, payload, function (res) {
                if (res.success) {
                    ...
                    Swal.fire('Error', res.data || 'Failed to save', 'error');
                }
