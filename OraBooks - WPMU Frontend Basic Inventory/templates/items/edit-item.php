...
if (!$item_id) {
    ...
    echo '<strong class="font-bold">Error!</strong> <span class="block sm:inline">Invalid item ID.</span>';
    echo '</div>';
    return;
}

// Fetch item data
$item = $wpdb->get_row($wpdb->prepare("SELECT * FROM $items_table WHERE id=%d", $item_id));

if (!$item) {
    echo '<div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">';
    echo '<strong class="font-bold">Error!</strong> <span class="block sm:inline">Item not found.</span>';
    echo '</div>';
    return;
}
...
$warehouse_item = $wpdb->get_row($wpdb->prepare("SELECT * FROM $warehouse_items_table WHERE item_id=%d LIMIT 1", $item_id));
?>
...
<div class="p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div class="flex items-center">
            <div
                class="w-12 h-12 bg-amber-100 text-amber-600 rounded-lg flex items-center justify-center mr-4 shadow-sm font-bold">
                <i class="fa-solid fa-pen-to-square text-xl"></i>
            </div>
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Edit Item</h1>
                <p class="text-sm text-gray-500 mt-1">Update item information and stock details</p>
            </div>
        </div>
    ...
    <form id="edit-item-form" class="space-y-6" enctype="multipart/form-data">
        <input type="hidden" name="action" value="frontend_update_item">
        ...
        <input type="hidden" name="store_id" value="<?php echo intval($item->store_id); ?>">
        <input type="hidden" name="existing_image" value="<?php echo esc_url($item->item_image); ?>">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="item_code" value="<?php echo esc_attr($item->item_code); ?>"
                ...
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Item Name <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="item_name" value="<?php echo esc_attr($item->item_name); ?>" required
                        class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                </div>
                    ...
                    <div class="flex">
                        <select name="brand_id"
                            ...
                            <?php foreach ($brands as $brand): ?>
                                <option value="<?php echo $brand->id; ?>" <?php selected($item->brand_id, $brand->id); ?>>
                    ...
                    <div class="flex">
                        <select name="category_id" required
                            ...
                            <?php foreach ($categories as $cat): ?>
                                <option value="<?php echo $cat->id; ?>" <?php selected($item->category_id, $cat->id); ?>>
                    ...
                    <div class="flex">
                        <select name="unit_id" required
                            ...
                            <?php foreach ($units as $unit): ?>
                                <option value="<?php echo $unit->id; ?>" <?php selected($item->unit_id, $unit->id); ?>>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="sku" value="<?php echo esc_attr($item->sku); ?>"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-gray-700 text-sm font-bold mb-2">HSN</label>
                        <input type="text" name="hsn" value="<?php echo esc_attr($item->hsn); ?>"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="alert_qty" value="<?php echo esc_attr($item->alert_qty); ?>"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500"
                            min="0">
                    </div>
                    <div>
                        <label class="block text-gray-700 text-sm font-bold mb-2">Seller Points</label>
                        <input type="text" name="seller_points" value="<?php echo esc_attr($item->seller_points); ?>"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="custom_barcode" value="<?php echo esc_attr($item->custom_barcode); ?>"
                        class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3"
                        class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500"><?php echo esc_textarea($item->description); ?></textarea>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <small class="text-red-500">Max 1000x1000px, 1MB</small>
                    <div id="image-preview"
                        class="mt-4 w-32 h-32 rounded border border-gray-300 overflow-hidden <?php echo $item->item_image ? '' : 'hidden'; ?>">
                        <img src="<?php echo esc_url($item->item_image); ?>" alt="Preview"
            ...
            <div class="bg-white p-6 rounded-lg shadow-md border border-gray-200 h-fit">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="discount_type"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                            <option value="Percentage" <?php selected($item->discount_type, 'Percentage'); ?>>
                                Percentage(%)</option>
                            <option value="Fixed" <?php selected($item->discount_type, 'Fixed'); ?>>Fixed</option>
                        </select>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-gray-700 text-sm font-bold mb-2">Price (excl. Tax) <span
                                class="text-red-500">*</span></label>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="purchase_price" id="purchase_price" readonly
                        ...
                        <select name="tax_id" id="tax_id"
                            ...
                            <?php foreach ($taxes as $tax): ?>
                                <option value="<?php echo $tax->id; ?>" data-percent="<?php echo esc_attr($tax->tax); ?>"
                                    <?php selected($item->tax_id, $tax->id); ?>><?php echo esc_html($tax->tax_name); ?>
                                    (<?php echo esc_html($tax->tax); ?>%)</option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="tax_type" id="tax_type"
                        class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                        <option value="Inclusive" <?php selected($item->tax_type, 'Inclusive'); ?>>Inclusive</option>
                        <option value="Exclusive" <?php selected($item->tax_type, 'Exclusive'); ?>>Exclusive</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="profit_margin" id="profit_margin"
                        value="<?php echo esc_attr($item->profit_margin); ?>"
                        class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="sales_price" id="sales_price"
                            value="<?php echo esc_attr($item->sales_price); ?>"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <select name="warehouse_id"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                            <?php
                            // Get the current warehouse ID from either warehouse_item table or item's warehouse_id field
                            $current_warehouse_id = 0;
                            if ($warehouse_item && $warehouse_item->warehouse_id) {
                                $current_warehouse_id = $warehouse_item->warehouse_id;
                            ...
                            foreach ($warehouses as $warehouse):
                                ?>
                                <option value="<?php echo $warehouse->id; ?>" <?php selected($current_warehouse_id, $warehouse->id); ?>><?php echo esc_html($warehouse->warehouse_name); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="number" step="0.01" name="adjustment_qty"
                            value="<?php echo esc_attr($item->stock); ?>"
                            class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="item_type"
                        class="w-full bg-white text-gray-700 border border-gray-300 rounded py-2 px-3 focus:outline-none focus:border-blue-500">
                        <option value="Single" <?php selected($item->item_group, 'Single'); ?>>Single</option>
                        <option value="Variants" <?php selected($item->item_group, 'Variants'); ?>>Variants</option>
                    </select>
                </div>
                ...
                <div class="mt-8">
                    <button type="submit" id="update-btn"
                        class="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-3 px-4 rounded focus:outline-none focus:shadow-outline transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
...
<script>
    jQuery(document).ready(function ($) {
        ...
        function calculatePricing() {
            ...
            // Only auto-fill MRP if it's identical to price (meaning it hasn't been manually changed) OR if we are just initializing
            // For simplicity, let's just auto-fill it on price change as requested.
        ...
        // Run once on load to initialize Purchase Price
        calculatePricing();
        ...
        $('#edit-item-form').on('submit', function (e) {
            ...
            $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin mr-2"></i> Updating...');
