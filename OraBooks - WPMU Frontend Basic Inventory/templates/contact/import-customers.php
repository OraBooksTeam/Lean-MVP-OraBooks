...
<div class="p-6">
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div class="flex items-center">
            <div class="w-12 h-12 bg-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center mr-4 shadow-sm font-bold">
                <i class="fa-solid fa-file-import text-xl"></i>
            </div>
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Import Customers</h1>
                <p class="text-sm text-gray-500 mt-1">Bulk upload customers using a CSV file</p>
            </div>
        </div>
        <a href="<?php echo esc_url(add_query_arg('view', 'customers')); ?>" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-gray-600 to-gray-700 text-white rounded-xl hover:from-gray-700 hover:to-gray-800 transition-all font-bold text-xs uppercase tracking-widest shadow-lg shadow-gray-200 hover:shadow-gray-300 active:scale-95">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 mb-6">
                <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center">
                    <i class="fa-solid fa-cloud-arrow-up mr-2 text-indigo-500"></i>
                    Upload CSV File
                </h3>
                
                <form id="import-customers-form" enctype="multipart/form-data" class="space-y-6">
                    <input type="hidden" name="action" value="frontend_import_customers">
                    <input type="hidden" name="security" value="<?php echo wp_create_nonce('frontend_ajax_nonce'); ?>">
                    
                    <div class="flex items-center justify-center w-full">
                        <label for="customer_csv" class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-2xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition-all hover:border-indigo-400 group">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <div class="w-16 h-16 bg-indigo-50 text-indigo-500 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                                ...
                                <div id="file-name-display" class="mt-4 text-indigo-600 font-medium hidden"></div>
                            </div>
                            <input id="customer_csv" name="customer_csv" type="file" accept=".csv" class="hidden" required />
                        </label>
                    </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-gradient-to-br from-indigo-600 to-violet-700 p-6 rounded-2xl shadow-xl text-white mb-6">
                ...
                <a href="<?php echo admin_url('admin-ajax.php?action=frontend_download_customer_template'); ?>" class="block w-full bg-white text-indigo-600 text-center py-3 px-4 rounded-xl font-bold hover:bg-indigo-50 transition-colors shadow-lg">
            ...
            <div class="bg-white p-6 rounded-2xl shadow-md border border-gray-100">
                ...
                <ul class="space-y-3 text-sm text-gray-600">
                    <li class="flex items-start gap-3">
                        ...
                        <span><strong>Customer Name</strong> is mandatory for every row.</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <i class="fa-solid fa-circle-info text-blue-500 mt-1"></i>
                        <span>Every customer will get a unique code (e.g. CUS-000001).</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <i class="fa-solid fa-circle-exclamation text-amber-500 mt-1"></i>
                        <span>For <strong>Credit Limit</strong>, use -1 for No Limit.</span>
                    </li>
                </ul>
            </div>
        </div>
...
jQuery(document).ready(function($) {
    ...
    $('#customer_csv').on('change', function(e) {
    ...
    // Form Submission
    $('#import-customers-form').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    setTimeout(function() {
                        window.location.href = '<?php echo esc_url(add_query_arg('view', 'customers')); ?>';
                    }, 2000);
                } else {
