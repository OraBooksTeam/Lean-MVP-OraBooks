...
if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['save_store_nonce'] ) && wp_verify_nonce( $_POST['save_store_nonce'], 'save_store_action' ) ) {
    ...
    $data = [
        ...
        'default_sales_discount' => $default_sales_discount,
        'sales_invoice_format'   => $sales_invoice_format,
        'pos_invoice_format'     => $pos_invoice_format,
        'show_mrp_pos'           => $show_mrp_pos,
        'show_prev_balance'      => $show_prev_balance,
        'num_to_words_format'    => $num_to_words_format,
    ...
    if ( ! empty( $_FILES['signature_image']['name'] ) ) {
        ...
        if ( isset( $upload['url'] ) ) {
            $data['signature_image'] = esc_url_raw( $upload['url'] );
        }
    }
    ...
    if ( $table_columns ) {
        $data = array_intersect_key( $data, array_flip( $table_columns ) );
    }
    ...
    if ( $store_id ) {
        ...
        if ( $updated !== false ) {
            ...
            $redirect_url = add_query_arg( [ 'message' => 'updated' ], remove_query_arg( [ 'message' ] ) );
            echo '<script>window.location.href = "' . esc_url_raw( $redirect_url ) . '";</script>';
        } else {
            $err = $wpdb->last_error ? urlencode($wpdb->last_error) : 'Unknown_Update_Error';
            $redirect_url = add_query_arg( [ 'message' => 'error', 'err' => $err ], remove_query_arg( [ 'message' ] ) );
            echo '<script>window.location.href = "' . esc_url_raw( $redirect_url ) . '";</script>';
        }
    } else {
        // INSERT
        ...
        // Re-filter after adding date fields
        if ( $table_columns ) {
            $data = array_intersect_key( $data, array_flip( $table_columns ) );
        }
        ...
        if ( $inserted ) {
            ...
            $redirect_url = add_query_arg( [ 'message' => 'saved' ], remove_query_arg( [ 'message' ] ) );
            echo '<script>window.location.href = "' . esc_url_raw( $redirect_url ) . '";</script>';
        } else {
            $err = $wpdb->last_error ? urlencode($wpdb->last_error) : 'Unknown_Insert_Error';
            $redirect_url = add_query_arg( [ 'message' => 'error', 'err' => $err ], remove_query_arg( [ 'message' ] ) );
            echo '<script>window.location.href = "' . esc_url_raw( $redirect_url ) . '";</script>';
        }
    }
...
<div class="bg-white rounded-lg shadow-lg p-6">
    ...
    if ( isset( $_GET['message'] ) ) {
        if ( $_GET['message'] === 'updated' ) {
            ...
            $err_msg = isset($_GET['err']) ? urldecode($_GET['err']) : 'Unknown Error';
            echo '<div class="relative bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-4" role="alert">
                    <p>Failed to update store configuration. Database Error: ' . esc_html($err_msg) . '</p>
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <select name="default_account_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                                ...
                                <?php
                                $accounts = $wpdb->get_results( "SELECT id, account_name, account_code FROM {$wpdb->prefix}orabooks_ac_accounts WHERE (account_code LIKE '1000%' OR account_name LIKE '%bank%' OR account_name LIKE '%cash%') AND (status=1 OR status IS NULL) AND (delete_bit=0 OR delete_bit IS NULL) ORDER BY account_code ASC" );
