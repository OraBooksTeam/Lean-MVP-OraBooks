...
<!-- Item Modal -->
<div id="item-modal-pos" class="fixed inset-0 z-[999999] hidden overflow-y-auto" aria-labelledby="modal-title"
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div
            ...
            <form id="item-modal-pos-form" enctype="multipart/form-data">
                <input type="hidden" name="action" value="frontend_insert_item">
                <input type="hidden" name="security" value="<?php echo esc_attr($nonce); ?>">
                <input type="hidden" name="store_id" value="1">
                <input type="hidden" name="count_id" id="m_count_id_pos" value="">

                <div class="bg-white px-8 py-6">
                    <div class="flex items-center justify-between mb-8 border-b border-gray-100 pb-5">
                        <div class="flex items-center">
                            <div
                                class="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg mr-4">
                        ...
                        <button type="button"
                            class="w-10 h-10 flex items-center justify-center rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-all close-item-modal-pos">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-5">
                        ...
                        <div class="md:col-span-3">
                            <h4 class="text-xs font-bold text-blue-600 uppercase tracking-[0.1em] mb-1">Basic
                                Information</h4>
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="text" name="item_code" id="m_item_code_pos" readonly
                                class="w-full rounded-xl border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed py-2 px-2.5 font-mono text-sm"
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="item_name" required
                                class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all py-2 px-2.5"
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Unit <span
                                    class="text-red-500">*</span></label>
                            <select name="unit_id" required
                                ...
                                <?php foreach ($units as $unit): ?>
                                    <option value="<?php echo $unit->id; ?>">
                                        <?php echo esc_html($unit->unit_name); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="md:col-span-3 mt-4">
                            <h4 class="text-xs font-bold text-amber-600 uppercase tracking-[0.1em] mb-1">Discount
                                Settings</h4>
                        ...
                        <!-- Section: Pricing -->
                        <div class="md:col-span-3 mt-4">
                            <h4 class="text-xs font-bold text-emerald-600 uppercase tracking-[0.1em] mb-1">Pricing & Tax
                            </h4>
                            <div class="h-1 w-12 bg-emerald-600 rounded-full"></div>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Price (Excl. Tax) <span
                                    class="text-red-500">*</span></label>
                            <input type="number" step="0.01" name="price" id="price_pos" required
                                class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all py-2 px-2.5"
                                placeholder="0.00">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Tax <span
                                    class="text-red-500">*</span></label>
                            <select name="tax_id" id="tax_id_pos"
                                class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all py-2 px-2.5">
                                <option value="" data-percent="0">- Select Tax -</option>
                                <?php foreach ($taxes as $tax): ?>
                                    <option value="<?php echo $tax->id; ?>"
                                        data-percent="<?php echo esc_attr($tax->tax); ?>">
                                        <?php echo esc_html($tax->tax_name); ?> (<?php echo esc_html($tax->tax); ?>%)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="tax_type" id="tax_type_pos"
                                class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all py-2 px-2.5">
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="purchase_price" id="purchase_price_pos" readonly
                                class="w-full rounded-xl border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed py-2 px-2.5"
                                placeholder="0.00">
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Profit Margin (%)</label>
                            <input type="number" step="0.01" name="profit_margin" id="profit_margin_pos"
                                class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all py-2 px-2.5"
                                placeholder="0.00">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="sales_price" id="sales_price_pos"
                                class="w-full rounded-xl border-gray-200 focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 shadow-sm transition-all py-2 px-2.5"
                                placeholder="0.00">
                        </div>
                        ...
                        <!-- Section: Stock -->
                        <div class="md:col-span-3 mt-4">
                            <h4 class="text-xs font-bold text-orange-600 uppercase tracking-[0.1em] mb-1">Stock & Others
                            </h4>
                        ...
                        <div class="md:col-span-1">
                            ...
                            <input type="file" name="item_image" id="m_item_image" accept="image/*"
                ...
                <div class="bg-gray-50 px-8 py-5 flex items-center justify-end gap-4 border-t border-gray-100">
                    <button type="button"
                        class="close-item-modal-pos px-6 py-2.5 text-sm font-bold text-gray-600 hover:text-gray-900 border border-gray-200 rounded-xl hover:bg-white hover:shadow-sm transition-all">
                        Cancel
                    </button>
                    <button type="submit" id="save-item-modal-pos-btn"
                        class="px-8 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 text-sm font-bold shadow-lg shadow-blue-500/30 transition-all flex items-center">
...
<script>
    jQuery(document).ready(function ($) {
        ...
        const form = $('#item-modal-pos-form');
        ...
        $(document).on('click', '.close-item-modal-pos, #item-modal-pos-overlay', closeItemModal);

        // Auto Calculations
        function calculatePricing() {
            let price = parseFloat($('#price_pos').val()) || 0;
            let taxPercent = parseFloat($('#tax_id_pos option:selected').data('percent')) || 0;
            let taxType = $('#tax_type_pos').val();
            let profitMargin = parseFloat($('#profit_margin_pos').val()) || 0;
            let purchasePrice = price;
            if (taxType === 'Exclusive' && taxPercent > 0) {
                purchasePrice = price + (price * taxPercent / 100);
            }
            $('#purchase_price_pos').val(purchasePrice.toFixed(2));
            let salesPrice = price;
            if (profitMargin > 0) {
                salesPrice = price + (price * profitMargin / 100);
            }
            $('#sales_price_pos').val(salesPrice.toFixed(2));
        }
        $('#price_pos, #profit_margin_pos, #tax_id_pos, #tax_type_pos').on('keyup change', calculatePricing);

        // Form Submission
        form.on('submit', function (e) {
            e.preventDefault();
            const btn = $('#save-item-modal-pos-btn');
    ...
    // Global function to generate item code
    function generateItemCode() {
        $.ajax({
            url: '<?php echo admin_url("admin-ajax.php"); ?>',
            type: 'POST',
            data: { action: 'generate_item_code', security: '<?php echo $nonce; ?>' },
            success: function (res) {
                // console.log(res);
                if (res.success) {
                    $('#m_item_code_pos').val(res.data.item_code);
                    $('#m_count_id_pos').val(res.data.count_id);
                }
            }
        });
    }
