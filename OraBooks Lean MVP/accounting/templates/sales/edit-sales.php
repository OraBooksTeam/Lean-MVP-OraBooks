    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 md:mb-6 gap-3 md:gap-4">
        <div class="flex items-center">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Modify existing sales invoice
                    (<?php echo esc_html($sale->sales_code); ?>)</p>
            </div>
        </div>
    ...
    <form id="sales-form" class="space-y-3 md:space-y-6 relative">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
                <div>
                    ...
                    <div class="flex rounded-lg shadow-sm group">
                        <span
                            class="inline-flex items-center px-4 rounded-l-lg border border-r-0 border-gray-300 bg-gray-100 text-gray-500 transition-colors min-h-[35px] md:min-h-0">
                            <i class="fa-solid fa-hashtag"></i>
                        </span>
                        <input type="text" name="sales_code" id="sales_code"
                            value="<?php echo esc_attr($sale->sales_code); ?>" readonly
                            class="flex-1 block w-full rounded-none rounded-r-lg border-gray-300 bg-gray-100 text-gray-600 cursor-not-allowed focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11 px-3 text-sm min-h-[35px] md:min-h-0">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="warehouse_id" id="warehouse_id" required
                        ...
                        <?php foreach ($warehouses as $w): ?>
                            <option value="<?php echo esc_attr($w->id); ?>" <?php selected($sale->warehouse_id, $w->id); ?>>
                ...
                <div>
                    ...
                    <input type="text" name="reference_no" id="reference_no"
                        value="<?php echo esc_attr($sale->reference_no); ?>"
                ...
                <div>
                    ...
                    <input type="date" name="sales_date" id="sales_date"
                        value="<?php echo esc_attr($sale->sales_date); ?>"
                        class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11 px-3">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                    <input type="date" name="due_date" id="due_date" value="<?php echo esc_attr($sale->due_date); ?>"
                        class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 shadow-sm h-10 md:h-11 px-3">
                </div>
                ...
                <div>
                    ...
                    <select name="sales_status" id="sales_status"
                        ...
                        <option value="Delivered" <?php echo ($sale->sales_status === 'Delivered') ? 'selected' : ''; ?>>
                            Delivered</option>
                        <option value="Pending" <?php echo ($sale->sales_status === 'Pending') ? 'selected' : ''; ?>>
                            Pending</option>
                        <option value="Ordered" <?php echo ($sale->sales_status === 'Ordered') ? 'selected' : ''; ?>>
                            Ordered</option>
                    </select>
                </div>
            </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
            <div
                ...
                <button type="button" id="add-blank-row-accounting"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse min-w-[1000px]" id="items-table">
                    ...
                    <tbody id="items-tbody-accounting" class="divide-y divide-gray-100">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" step="0.01" name="other_charges_input" id="other_charges_input"
                                value="<?php echo esc_attr($sale->other_charges_input); ?>"
                            ...
                            <select name="other_charges_tax_id" id="other_charges_tax_id"
                                ...
                                <?php foreach ($taxes as $t): ?>
                                    <option value="<?php echo esc_attr($t->id); ?>"
                                        data-percent="<?php echo esc_attr($t->tax); ?>" <?php selected($sale->other_charges_tax_id, $t->id); ?>>
                                        <?php echo esc_html($t->tax_name . ' (' . $t->tax . '%)'); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex rounded-md shadow-sm">
                            <input type="number" step="0.01" id="discount_to_all_input" name="discount_to_all_input"
                                value="<?php echo esc_attr($sale->discount_to_all_input); ?>"
                                class="w-[60%] rounded-l-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500 h-10 md:h-11 px-3">
                            <select id="discount_to_all_type" name="discount_to_all_type"
                                ...
                                <option value="Fixed" <?php selected($sale->discount_to_all_type, 'Fixed'); ?>>Fixed
                                    Amount</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="sales_note" id="sales_note" rows="3"
                            ...
                            placeholder="Add a note..."><?php echo esc_textarea($sale->sales_note); ?></textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-3 mb-6">
                    <div class="flex justify-between text-sm text-gray-600">
                        ...
                        <span class="font-bold text-gray-800"
                            id="subtotal_text"><?php echo number_format($sale->subtotal, 2); ?></span>
                    </div>
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Other Charges</span>
                        <span class="font-bold text-gray-800"
                            id="other_charges_text"><?php echo number_format($sale->other_charges_amt, 2); ?></span>
                    </div>
                    <div class="flex justify-between text-sm text-red-600">
                        <span>Discount on All</span>
                        <span class="font-bold">- <span
                                id="discount_all_text"><?php echo number_format($sale->tot_discount_to_all_amt, 2); ?></span></span>
                    </div>
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Tax Total</span>
                    ...
                    <div class="flex justify-between text-sm text-gray-600">
                        <span>Round Off</span>
                        <span class="font-bold text-gray-800"
                            id="round_off_text"><?php echo number_format($sale->round_off, 2); ?></span>
                    </div>
                    <div
                        ...
                        <span class="text-2xl text-green-600"
                            id="grand_total_text"><?php echo number_format($sale->grand_total, 2); ?></span>
                    </div>
                </div>
                ...
                <div class="bg-white p-3 md:p-4 rounded-lg border border-blue-100 shadow-sm mb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        <div>
                            ...
                            <div
                                class="p-2 md:py-2.5 bg-gray-50 rounded border text-sm font-semibold h-10 md:h-11 flex items-center">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                        <div>
                            ...
                            <div class="flex gap-1 w-full">
                                ...
                                <button type="button" id="edit-add-payment-type-btn"
                        ...
                        <div>
                            ...
                            <input type="number" step="0.01" name="payment_amount" id="payment_amount"
                                value="<?php echo esc_attr($sale->paid_amount); ?>" placeholder="0.00"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                ...
                                <div class="flex-1 relative obn-select2-account-parent">
                                    <select id="account_id" name="account_id"
                                        ...
                                        <?php foreach ($accounts as $acc): ?>
                                            <option value="<?php echo esc_attr($acc->id); ?>" <?php selected($existing_payment ? $existing_payment->account_id : 0, $acc->id); ?>>
                                                <?php echo esc_html(($acc->account_code ? $acc->account_code . ' - ' : '') . $acc->account_name); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                                <button type="button" id="edit-add-account-btn"
                                    class="w-10 h-10 md:h-11 flex-shrink-0 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 border border-blue-200 transition-colors flex items-center justify-center"
                        ...
                        <div>
                            ...
                            <textarea name="payment_note" id="payment_note" rows="1"
                                class="w-full rounded border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500 min-h-[44px] leading-tight py-2 px-3"><?php echo esc_textarea($existing_payment ? $existing_payment->payment_note : ''); ?></textarea>
                        </div>
                    </div>
                </div>

                <button type="button" id="save-sale"
                    class="w-full py-4 md:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all font-bold text-lg shadow-md flex justify-center items-center">
...
<script>
    jQuery(document).ready(function ($) {
        ...
        function initPaymentTypeSelect2() {
            if ($.fn.select2) {
                const $paymentSelect = $('#payment_type_id');
        ...
        // Add Blank Row
        $('#add-blank-row-accounting').on('click', function () {
            addItemRow({
                id: '',
                item_name: '',
                item_code: '',
        ...
        function addItemRow(item) {
            ...
            // After Select2 init, set the account value so it displays correctly
            if (item.account_id) {
                row.find('.row-account-id').val(item.account_id).trigger('change');
            } else if (item.sales_account_id) {
                row.find('.row-account-id').val(item.sales_account_id).trigger('change');
            } else {
                ...
                if (salesAcc) row.find('.row-account-id').val(salesAcc).trigger('change');
            }

            $('#items-tbody-accounting').append(row);
            ...
            row.find('.row-name').autocomplete({
                ...
                select: function (evt, ui) {
                    ...
                    if (data.sales_account_id) {
                        r.find('.row-account-id').val(data.sales_account_id).trigger('change');
                    } else {
                        const salesAcc = r.find('.row-account-id option').filter(function () {
                            const text = $(this).text().toLowerCase();
                            return text.indexOf('4000') !== -1 || text === 'sales' || text.endsWith(' - sales');
                        }).val();
                        if (salesAcc) r.find('.row-account-id').val(salesAcc).trigger('change');
                    }
            ...
            row.find('.remove-row').on('click', function () {
                ...
                if ($('#items-tbody-accounting tr').length === 0 || $('#items-tbody-accounting tr:visible').length === 0) $('#empty-row-msg').show();
                recalc();
            });
        }
        ...
        function recalc() {
            ...
            $('#items-tbody-accounting tr:not(#empty-row-msg)').each(function () {
        ...
        $('#save-sale').on('click', function () {
            ...
            if ($('#items-tbody-accounting tr:not(#empty-row-msg)').length === 0) { alert('Add at least one item'); return; }
            ...
            $.post(ajaxurl, payload, function (res) {
                if (res.success) {
                    ...
                    alert('Error: ' + ((res.data && res.data.message) || res.data || 'Failed to update'));
                }
            }, 'json').fail(function (xhr) {
                alert('Error: ' + getAjaxErrorMessage(xhr, 'Failed to update sale.'));
            }).always(function () {
        ...
        $('#edit-add-payment-type-btn').on('click', function () {
        ...
        $('#edit-add-account-btn').on('click', function () {
