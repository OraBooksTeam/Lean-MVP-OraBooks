...
<div class="container mx-auto px-4 py-6 text-gray-800">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 no-print">
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">
                    <?php echo $currency . ' ' . number_format($stats->total_debit ?? 0, 2); ?></p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 flex items-center justify-between">
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">
                    <?php echo $currency . ' ' . number_format($stats->total_credit ?? 0, 2); ?></p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100 no-print">
        <form id="search-form" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Source
                    Type</label>
            ...
            <div>
                <button type="submit"
                    class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-4 rounded-lg transition-all duration-200 shadow-sm">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-4 no-print">
        ...
        <div class="relative w-full md:w-1/3">
            ...
            <input type="search" id="tableSearch"
                ...
                placeholder="Filter journals in view...">
        </div>

        <div class="flex justify-end gap-2 flex-wrap w-full md:w-auto">
            <button id="journalPrintBtn"
                class="text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-4 py-2 transition-all shadow-sm">
                <i class="fa-solid fa-print mr-1"></i> Print
            </button>
            <button id="journalPdfBtn"
                class="text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-4 py-2 transition-all shadow-sm">
                <i class="fa-solid fa-file-pdf mr-1 text-red-600"></i> PDF
            </button>
            <button id="journalExcelBtn"
                class="text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-4 py-2 transition-all shadow-sm">
            ...
            <div class="relative">
                <button id="journalColumnToggleBtn"
                    class="text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-4 py-2 transition-all shadow-sm">
                    <i class="fa-solid fa-columns mr-1"></i> Columns
                </button>
                <div id="journalColumnDropdown"
                    class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 overflow-hidden">
                    <div class="p-4 space-y-3">
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="0" checked> #
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="1" checked> Date
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="2" checked> Journal ID
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="3" checked> Reference
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="4" checked> Account
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="5" checked> Description
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="6" checked> Debit
                        </label>
                        <label
                            class="flex items-center cursor-pointer hover:bg-gray-50 p-1 rounded transition-colors text-sm font-medium text-gray-700">
                            <input type="checkbox" class="column-toggle rounded border-gray-300 text-blue-600 mr-3"
                                data-column="7" checked> Credit
                        </label>
                    </div>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody id="journal-table-body" class="bg-white divide-y divide-gray-200">
                    <tr>
                        <td colspan="8" class="px-6 py-10 text-center text-gray-500">Click Fetch Journals to load data
                        </td>
                    </tr>
                </tbody>
...
<script>
    jQuery(document).ready(function ($) {
        ...
        $('#journalColumnToggleBtn').on('click', function (e) { e.stopPropagation(); $('#journalColumnDropdown').toggleClass('hidden'); });
        $(document).on('click', function (e) { if (!$(e.target).closest('#journalColumnToggleBtn, #journalColumnDropdown').length) $('#journalColumnDropdown').addClass('hidden'); });

        $('.journalreport-column-toggle').on('change', function () {
        ...
        $('#journalPrintBtn').on('click', function () {
        ...
        $('#journalPdfBtn').on('click', function () {
        ...
        $('#journalExcelBtn').on('click', function () {
