...
class OBN_Expenses
{
    ...
    public static function insert_expense()
    {
        ...
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            OBN_Fiscal_Period_Posting_Guard::assert_can_post_or_fail($expense_date);
        }
        ...
        $expense = $wpdb->get_row($wpdb->prepare("SELECT expense_date FROM $expense_table WHERE id = %d", $expense_id));
        if (!$expense) {
    ...
    public static function update_expense()
    {
        ...
        if (class_exists('OBN_Fiscal_Period_Posting_Guard')) {
            $existing_expense = $wpdb->get_row($wpdb->prepare("SELECT expense_date FROM {$wpdb->prefix}orabooks_db_expense WHERE id = %d", $expense_id));
            if (!$existing_expense) {
                wp_send_json_error('Expense not found');
            }
