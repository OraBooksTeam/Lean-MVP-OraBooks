...
class OBN_Activator
{
    ...
    public static function activate()
    {
        ...
        // 93 Table: fiscal_periods (SL-304 Fiscal Period & Lock Governance)
        $table93 = $wpdb->prefix . 'fiscal_periods';
        $sql93 = "CREATE TABLE {$table93} (
        ...
        // 94 Table: orabooks_ac_audit_events
        $table94 = $wpdb->prefix . 'orabooks_ac_audit_events';
        $sql94 = "CREATE TABLE {$table94} (
            `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
            `org_id` bigint(20) unsigned NOT NULL,
        ...
        $default_menus_acc = [
            ...
            ["module" => "accounting", "parent" => 0, "menu_title" => "Purchase", "menu_slug" => "purchase", "icon" => "fa-solid fa-bag-shopping", "sort_order" => 6, "status" => 1],
            ["module" => "accounting", "parent" => 0, "menu_title" => "Quotation", "menu_slug" => "quotation", "icon" => "fa-solid fa-file-invoice-dollar", "sort_order" => 7, "status" => 1],
            ["module" => "accounting", "parent" => 0, "menu_title" => "Expense", "menu_slug" => "expense", "icon" => "fa-solid fa-receipt", "sort_order" => 8, "status" => 1],
            ["module" => "accounting", "parent" => 0, "menu_title" => "Coupons", "menu_slug" => "coupons", "icon" => "fa-solid fa-tags", "sort_order" => 9, "status" => 1],
            ["module" => "accounting", "parent" => 0, "menu_title" => "Assets", "menu_slug" => "assets", "icon" => "fa-solid fa-building-circle-check", "sort_order" => 10, "status" => 1],
            ["module" => "accounting", "parent" => 0, "menu_title" => "Reimbursements", "menu_slug" => "reimbursements", "icon" => "fa-solid fa-hand-holding-dollar", "sort_order" => 11, "status" => 1],
            ["module" => "accounting", "parent" => 0, "menu_title" => "Acc. Reports", "menu_slug" => "acc-reports", "icon" => "fa-solid fa-chart-line", "sort_order" => 12, "status" => 1],
        ];
        ...
        foreach ($default_menus_acc as $menu) {
            ...
            if ($menu['menu_title'] == 'Setting') {
                $submenus = [
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Store Profile", "menu_slug" => "store-profile", "icon" => "fa-solid fa-store", "sort_order" => 1, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Warehouse", "menu_slug" => "warehouse", "icon" => "fa-solid fa-warehouse", "sort_order" => 2, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Currency", "menu_slug" => "setting-currency-list", "icon" => "fa-solid fa-coins", "sort_order" => 3, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Tax", "menu_slug" => "setting-tax-list", "icon" => "fa-solid fa-percent", "sort_order" => 4, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Payment types", "menu_slug" => "setting-payment-types-list", "icon" => "fa-solid fa-credit-card", "sort_order" => 5, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Add Features", "menu_slug" => "add-features", "icon" => "fa-solid fa-plus-circle", "sort_order" => 6, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "User Permissions", "menu_slug" => "setting-user-permissions", "icon" => "fa-solid fa-user-shield", "sort_order" => 7, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Fiscal Periods", "menu_slug" => "fiscal-periods", "icon" => "fa-solid fa-lock", "sort_order" => 8, "status" => 1],
                ];
                ...
                $submenus = [
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "View Purchase", "menu_slug" => "view-purchase", "icon" => "fa-solid fa-file-invoice-dollar", "sort_order" => 1, "status" => 1],
                    ["module" => "accounting", "parent" => $parent_id, "menu_title" => "Add Purchase", "menu_slug" => "add-purchase", "icon" => "fa-solid fa-bag-shopping", "sort_order" => 2, "status" => 1],
                ];
        ...
        // Initialize oraBooks User Permissions for Admin Users with Accounting Module IDs
        $table_user_permissions = $wpdb->prefix . 'orabooks_user_permissions';

        // Fetch all accounting menu IDs from sidebar table
        $accounting_sidebar_ids = $wpdb->get_col($wpdb->prepare("SELECT id FROM $table_sidebar WHERE module = %s", 'accounting'));

        if (!empty($accounting_sidebar_ids)) {
            ...
            foreach ($admin_ids as $admin_id) {
                ...
                $existing_permission = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table_user_permissions WHERE user_id = %d", $admin_id));

                if ($existing_permission) {
                    // Merge existing sidebar_ids with accounting IDs
                    $existing_ids = json_decode($existing_permission->sidebar_ids, true);
                    if (!is_array($existing_ids)) {
                        $existing_ids = [];
                    }
                    $merged_ids = array_values(array_unique(array_merge(
                        array_map('intval', $existing_ids),
                        array_map('intval', $accounting_sidebar_ids)
                    )));
                    $wpdb->update(
                        $table_user_permissions,
                        ['sidebar_ids' => json_encode($merged_ids)],
                        ['id' => $existing_permission->id]
                    );
                } else {
                    // No existing permissions, create new row with accounting IDs only
                    $wpdb->insert($table_user_permissions, [
                        'user_id' => $admin_id,
                        'sidebar_ids' => json_encode(array_map('intval', $accounting_sidebar_ids))
                    ]);
                }
            }
        }
    ...
    /**
     * Apply incremental schema updates for existing installs.
     */
    public static function maybe_upgrade_schema()
    {
        ...
        if ($wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s', $purchase_items_table)) !== $purchase_items_table) {
            return;
        }

        $has_account_id = $wpdb->get_var($wpdb->prepare(
            "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND COLUMN_NAME = 'account_id' LIMIT 1",
            DB_NAME,
            $purchase_items_table
        ));

        if (!$has_account_id) {
            $wpdb->query("ALTER TABLE {$purchase_items_table} ADD COLUMN `account_id` int(11) DEFAULT NULL AFTER `item_id`");
        }

        $has_return_qty = $wpdb->get_var($wpdb->prepare(
            "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = %s AND TABLE_NAME = %s AND COLUMN_NAME = 'return_qty' LIMIT 1",
            DB_NAME,
            $purchase_items_table
        ));

        if (!$has_return_qty) {
            $wpdb->query("ALTER TABLE {$purchase_items_table} ADD COLUMN `return_qty` double(20,2) DEFAULT NULL AFTER `purchase_qty`");
        }
    }
