...
import { CheckCircle2, ExternalLink, Receipt, RefreshCw, XCircle } from 'lucide-react';
...
export default function ExpensesLiveTestPage() {
  ...
  const runLiveCheck = async () => {
    setRunning(true);
    setError('');
    const res = await api.expensesLiveCheck();
    if (res.error) {
      setError(res.error);
      setResult(null);
    } else {
  ...
  return (
    <ClientShell
      title="SL-028 Live Test"
      eyebrow="Expenses OCR — production smoke test"
      organization={context?.organization}
    >
      <div className="space-y-5">
        <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50/80 p-4 text-sm text-amber-950">
          <Receipt className="mt-0.5 h-4 w-4 shrink-0" />
          <p>
            Production smoke test for SL-028 expense receipt OCR. Runs against the deployed plugin — no local
            environment required. Save PHP changes directly to the plugin folder; run{' '}
            <code className="text-xs">build-live.cmd</code> after UI edits, then hard-refresh this page.
          </p>
        </div>
            ...
            <div className="border-b border-border px-5 py-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <h2 className="font-bold text-ink">
                  {result.ok ? 'SL-028 live checks passed' : 'Some live checks need attention'}
                </h2>
              </div>
              {result.environment && (
                <p className="mt-2 text-xs text-slate-500">
                  Org #{String(result.environment.org_id ?? '—')} · OCR provider:{' '}
                  {String(result.environment.ocr_provider ?? 'unknown')} · Confidence threshold:{' '}
