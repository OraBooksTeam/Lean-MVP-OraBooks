/** SL-008 — client-side TLS and outbound URL guards (mirrors backend SSRF/HTTPS policy). */
...
/** Redirect HTTP → HTTPS on non-local hosts (SL-008 §5.3). */
...
export function validateHttpsWebhookLines(text: string) {
  ...
  for (const line of parseUrlLines(text)) {
    ...
    if (!isSecureOrigin(url)) {
      errors.push(`HTTPS required (SL-008 / SSRF policy): ${line}`);
    }
  }
