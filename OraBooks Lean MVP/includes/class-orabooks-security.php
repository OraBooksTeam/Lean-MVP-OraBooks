 ...
 * and verification cron jobs with alerting.
 */
...
class OraBooks_Security {
 ...
 /** Centralized rate-limit configuration ( §5.4). */
 ...
 /** Default SSRF allowlist patterns ( §5.5). */
 ...
 /** Simple JSON input schemas for governance ( §5.3). */
 ...
 */
 ...
 /**
 * SSRF allowlist validation for outbound URLs ( §5.5).
 *
 ...
 */
 ...
 /**
 * JSON input schema validation helper ( §5.3).
 *
 * @return true|WP_Error
 */
...
: sprintf('Secret rotation OK (%d days until due)', max(0, self::SECRET_ROTATION_DAYS - $days_since));
 ...
 'message' => sprintf(
 __('JWT/encryption secrets have not been rotated in %d days. Review rotation policy.', 'orabooks'),
 $days_since
 ),
 'priority' => 'high',
