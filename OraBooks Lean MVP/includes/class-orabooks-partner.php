...
class OraBooks_Partner {
 ...
 //: Admin partner approval / rejection
 ...
 * Uses customers.is_active as the source of truth
 */
 ...
 if ($customers_exists) {
 // exists: use customers.is_active as truth source
 ...
 // Fallback: just count verified attributions (pre-)
 return (int) $wpdb->get_var($wpdb->prepare(
 ...
 */
 ...
 do_action('orabooks_partner_inactivity_reminder_sent', $p->user_id, [
 'days' => 330,
 'active_customer_count' => 0,
 ]);
 }
 ...
 if (class_exists('OraBooks_Notifications')) {
 do_action('orabooks_partner_code_inactivated', $p->user_id, [
 'reason' => '12 months no attribution and zero active customers',
 ]);
 }
 }
 ...
 // Low-activity reminder (dormant partners only — doctrine)
 ...
 */
 ...
 /**
 * Ensure caller belongs to a partner organization ( onboarding access).
 *
 ...
 */
 ...
 // ============================================================
 //: PARTNER DASHBOARD DATA (unified endpoint)
 // ============================================================

 /**
 * Get full partner dashboard data
 ...
 */
 ...
 // Active customer count (using customers.is_active as source of truth)
 $table_customers = OraBooks_Database::table('customers');
 ...
 if (!$customers_table_exists) {
 // Pre-: fallback to verified attribution count
 $active_customer_count = (int) $wpdb->get_var($wpdb->prepare(
 "SELECT COUNT(DISTINCT pa.customer_user_id)
 FROM {$table_attributions} pa
 WHERE pa.partner_user_id = %d
