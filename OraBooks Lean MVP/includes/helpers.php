...
/**
 * Lean MVP frontend page definitions ( locked build).
 *
...
function orabooks_get_lean_mvp_page_definitions() {
 ...
 'classification-test' => ['Classification Live Test', '[orabooks_classification_test]'],
 'expenses-test' => ['Expenses Live Test', '[orabooks_expenses_test]'],
...
/**
 * Whether an organization subdomain may be used for tenant context.
 */
...
/**
 * Frontend URL for accepting a team invitation.
 */
...
: orabooks_random_string(32);
 $username = orabooks_generate_username_from_email($user->email);	if (function_exists('is_multisite') && is_multisite() && function_exists('wpmu_create_user')) {
...
/**
 *: block tenant subdomain access when org status does not allow it.
 */
 ...
 */
function orabooks_multisite_uses_signup_activation() {	if (!function_exists('is_multisite') || !is_multisite() || !function_exists('get_site_option')) {
 return false;
 }
...
function orabooks_create_wp_user_for_registration($email, $password, $meta = []) {
 ...
 $username = orabooks_generate_username_from_email($email);	if (orabooks_multisite_uses_signup_activation() && function_exists('wpmu_signup_user')) {
 ...
 */
function orabooks_set_auth_token_cookie($token) {	if (empty($token) || headers_sent()) {
		return;
	}
...
function orabooks_clear_auth_token_cookie() {
 $path = defined('COOKIEPATH') && COOKIEPATH ? COOKIEPATH: '/';	$secure = is_ssl();
...
/**
 * Persist refresh token in an HTTP-only cookie.
 */
...
/**
 * Resolve user ID from a tier-selection JWT (: no session until org exists).
 */
...
function orabooks_destroy_auth_session($user_id = 0, $log = true, $set_landing_cookie = null) {
 ...
 if ($user_id <= 0) {	$user_id = orabooks_resolve_authenticated_user_id();
 }
...
/**
 * Validate input against schema; sends 400 JSON error on failure.
 *
...
/**
 * Fetch the newest unused invite for an email address ( invite-first onboarding).
 *
 * @return object|null
 */
 ...
 * Enforces membership and JWT org_id alignment ( §5.7).
 */
...
/**
 * Assert tenant access for non-admin API calls ( §5.7).
 *
...
/**
 * Request-scoped correlation ID for workflow tracing.
 */
