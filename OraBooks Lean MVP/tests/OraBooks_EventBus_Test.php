<?php
/**
 * Unit Tests for OraBooks_EventBus (SL-302)
 */
...
class OraBooks_EventBus_Test extends TestCase
{
    ...
    public function test_publish_writes_pending_outbox_event_with_default_version()
    {
        ...
        $this->assertStringContainsString('orabooks_outbox_messages', $captured[0]);
        $this->assertSame('journal_posted', $captured[1]['event_type']);
        $this->assertSame(55, $captured[1]['aggregate_id']);
        $this->assertSame('pending', $captured[1]['status']);
        $this->assertSame(1, json_decode($captured[1]['payload'], true)['event_version']);
    }
