...
const currentLayers = [
 ...
 ["Database — MySQL 8", "Accounting table গুলো {prefix}orabooks_* naming convention follow করে। Event table: {prefix}gob_*_tob । ACID transaction, FOR UPDATE lock ( workflow) এবং WordPress $wpdb integration-এর জন্য MySQL।"],
 ["Events — Transactional Outbox", "Business change-এর সাথে same transaction-এ event outbox-এ write হয়। Worker পরে consumer-দের deliver করে। Idempotency consumer_log দিয়ে ensure করা হয়।"],
 ["Queue — Async Jobs + WordPress Cron", "Background job (OCR, export, webhook, email) orabooks_async_jobs table-এ queue হয়। WP Cron every_minute schedule দিয়ে worker process করে। MVP-তে Redis ছাড়াই async processing সম্ভব।"],
 ["Workflow — State Engine", "Journal, Invoice, Bill, Expense, Commission—সব record-এর status change একটিমাত্র entry point: OraBooks_Workflow::transition। DB lock, audit, event publish built-in।"],
 ...
 ["AI — Azure Document Intelligence + OpenAI/Azure OpenAI", "Receipt OCR, Voice, Classification । API key না থাকলে MVP stub fallback deterministic result দেয়।"],
 ["PWA — Service Worker + IndexedDB", "Mobile expense receipt offline queue । Service worker shell cache করে।"],
...
const domainModules = [
 ": Organization, subdomain, tier, data residency, partner/customer org types",
 ": Auth — register, login, Google OIDC, JWT, refresh token, email verify",
 ": RBAC/ABAC — deny-by-default permissions, cross-tenant guard",
 ": Team invites, role assignment",
 ": Journal posting engine — canonical ledger, hash chain",
 ": Approval gate — journal review workflow",
 ": Workflow state engine — invoice, bill, expense, commission states",
 ": Chart of Accounts",
 ": Customers & Invoices (AR)",
 ": Vendors & Bills (AP)",
 ": Expenses OCR + PWA offline receipts",
 ": Bank reconciliation",
 ": Inventory lite",
 ": Voice-to-text input",
 ...
 "/075: Financial & operational reports",
 ": PDF/CSV exports",
 ": CSV imports",
...
const securitySections = [
 ...
 why: "Accounting software-এ secret leak মানে financial data compromise। সব key encrypt করে এবং production-এ TLS enforce করে।",
 },
 {
...
const currentTableRows = [
 ...
 ["Events", " Outbox (MySQL)", "Domain Events", "Transactional consistency"],
 ["Queue", " + WP Cron", "Background Jobs", "MVP async without Redis infra"],
 ["Workflow", " State Engine", "Status Management", "Single write path, audit, locks"],
 ["AI", "Azure DI + OpenAI", "Intelligence", "Spec-aligned OCR/voice/classification"],
 ["Storage", "Local / WP Uploads", "Files", "MVP simplicity"],
 ["Auth", "JWT + OIDC + 2FA", "Security", " compliant"],
 ...
 ["Monitoring", " Internal Metrics", "Observability", "MVP health dashboard"],
...
function buildChildren {
 ...
 p(
 "OraBooks Lean MVP বর্তমানে WordPress Multisite-এর উপর একটি PHP plugin হিসেবে চলছে। UI-র জন্য React 19 + Vite + Tailwind 4 ব্যবহার করা হয়। Accounting data MySQL database-এ সংরক্ষিত হয়। থেকে পর্যন্ত specification অনুযায়ী module implement করা হয়েছে।",
 { size: 22 }
 ),
 ...
 " | | |",
 " v v v",
 " [MySQL] [ Outbox] [ Jobs + WP Cron]",
 ]),
 p("১.৩ Domain Modules ( → )", { heading: HeadingLevel.HEADING_2, size: 24, bold: true, color: ACCENT }),
...
...bullets([
 "WordPress plugin continue —,,, stabilize",
